...
- **React 16 constraints:** Latest React is 19.2.6 and latest BlueprintJS core is 6.12.1, but BlueprintJS 6 peers on React 18. A "latest everything" interpretation would force a React migration, which is outside a conservative dependency/test milestone unless explicitly accepted.
- **Jest major-version jump:** Moving from Jest 24 to Jest 30 can require transform, environment, fake timer, snapshot, and ESM changes.
...
- Whether "recommended latest" means all compatible latest versions or disruptive latest major versions even when they imply React/Blueprint migration.
