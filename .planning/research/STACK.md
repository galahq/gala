...
- Flow: `flow-bin` `0.87.0`, `flow-inspect`, `@babel/preset-flow`, and `eslint-plugin-flowtype`
...
| TypeScript | `6.0.3`, Node engine `>=14.17` | https://registry.npmjs.org/typescript/latest |
| flow-bin | `0.313.0`; project should remove Flow rather than update it | https://registry.npmjs.org/flow-bin/latest |
...
- Keep React 16.8 and BlueprintJS 4.x during this milestone. Latest React and BlueprintJS are intentionally out of scope because BlueprintJS 6 currently peers on React 18.
- Remove Flow from Node dependencies and source processing. Use TypeScript as the forward type-system target, with JSDoc allowed where keeping files as JavaScript reduces migration risk.
...
- Flow syntax is widespread across `app/javascript`, so removal needs source-by-source conversion or stripping, not only dependency deletion.
- A TypeScript-first target may require `allowJs`/`checkJs` staging before converting `.js`/`.jsx` files to `.ts`/`.tsx`.
