...
feature 'Customizing a deployment' do
  ...
  context 'as an instructor' do
    ...
    scenario 'it is possible to create and edit a new quiz' do
      ...
      find('.bp4-control-group .bp4-input').send_keys "Blue\nYellow\nRed\nGreen"
      click_button 'Deploy'
      expect(page).to have_content 'Please'
      all('.bp4-radio').first.click
      click_button 'Deploy'

      click_link 'Edit Quiz'
      expect(page).to have_selector '.bp4-input[value="What is the color of the sky?"]'
      click_button 'Add question'
      click_button 'Deploy'
      expect(page).to have_content 'Please'
      all('input.bp4-input').last.set('Why is it that color?')
      click_button 'Deploy'
      expect(page).to have_content 'Please'
