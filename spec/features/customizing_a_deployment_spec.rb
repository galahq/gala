...
feature 'Customizing a deployment' do
  ...
  context 'as an instructor' do
    ...
    scenario 'it is possible to create and edit a new quiz' do
      ...
      find('.bp6-control-group .bp6-input').send_keys "Blue\nYellow\nRed\nGreen"
      click_button 'Deploy'
      expect(page).to have_content 'Please'
      all('.bp6-radio').first.click
      click_button 'Deploy'

      click_link 'Edit Quiz'
      expect(page).to have_selector '.bp6-input[value="What is the color of the sky?"]'
      click_button 'Add question'
      click_button 'Deploy'
      expect(page).to have_content 'Please'
      all('input.bp6-input').last.set('Why is it that color?')
      click_button 'Deploy'
      expect(page).to have_content 'Please'
