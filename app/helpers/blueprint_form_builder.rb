...
class BlueprintFormBuilder < ActionView::Helpers::FormBuilder
  ...
  def form_group(method, label: nil, in_parens: nil, placeholder: nil,
    ...
    without_field_error_wrapper do
      classes = ['bp4-form-group'] + error_classes(method)
  ...
  def errors
    ...
    classes = %w[bp4-callout bp4-intent-danger bp4-icon-error form__callout]
  ...
  def check_box(method, **options)
    without_field_error_wrapper do
      @template.content_tag :label, class: %i[bp4-control bp4-checkbox] do
        content = ''.html_safe
        content << super(method, options)
        content << @template.content_tag(:span, '', class: %i[bp4-control-indicator])
  ...
  def radio_button(method, value, **options)
    without_field_error_wrapper do
      @template.content_tag :label, class: %i[bp4-control bp4-radio] do
        content = ''.html_safe
  ...
  def submit(*args, **kwargs)
    ...
    classes = %w[bp4-button]

    if !class_argument.delete('bp4-intent-none') &&
       !class_argument.one?(/^bp4-intent/)
      classes.push('bp4-intent-success')
    end
  ...
  def label_with_text_in_parens(method, label, in_parens)
    ...
    label method, contents, class: 'bp4-label'
  end
  ...
  def form_content(method, placeholder, helper_text, &block)
    @template.content_tag :div, class: 'bp4-form-content' do
      contents = ''.html_safe
      ...
      contents << if block_given?
                    ...
                    classes = %w[bp4-input bp4-fill] + error_classes
  ...
  def error_header
    @template.content_tag :h5, class: %w[bp4-callout-title] do
  ...
  def with_blueprint_file_input(method, instructions: nil, **options)
    label method, class: 'bp4-label' do
      contents = ''.html_safe
      classes = %w[bp4-file-input].concat Array(options.delete(:class))
  ...
  def file_input_span(instructions)
    @template.content_tag :span, class: %i[bp4-file-upload-input] do
