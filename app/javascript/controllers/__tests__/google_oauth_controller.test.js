...
describe('GoogleOauthController', () => {
  it('requires an email before starting Google authorization', () => {
    ...
    emailTarget.reportValidity = vi.fn()
    emailTarget.focus = vi.fn()
    const controller = { emailTarget, navigate: vi.fn() }
    const event = {
  ...
  it('navigates with only the normalized email from its target', () => {
    document.body.innerHTML = `
      <form data-controller="google-oauth">
    ...
    const destination = new URL(controller.navigate.mock.calls[0][0])
