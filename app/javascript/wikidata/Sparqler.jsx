...
import { InputGroup, Button, Intent, Card, Elevation, Spinner, Callout, MenuItem } from "@blueprintjs/core"
...
const Sparqler = () => {
  ...
  const handleClear = () => updateState({ query: "", results: null, error: null, inputIntent: Intent.NONE, loading: false })
  ...
  const renderResults = () =>
    results.map((result, index) => (
      <Card key={index} elevation={Elevation.TWO} style={{ marginBottom: "10px" }}>
        {result.image && <img src={result.image} alt={result.title || "Result"} style={{ maxWidth: "100px", float: "right", marginLeft: "10px" }} />}
        <h5>{result.label} ({result.qid})</h5>
