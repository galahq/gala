...
const Popover = styled(animated.div).attrs({
  className: 'bp4-popover bp4-popover-content-sizing',
})`
...
const Content = styled.div.attrs(p => ({
  className: `bp4-popover-content personas__choice persona--${p.persona}`,
}))`
  ${p =>
...
function Arrow ({ style, persona, placement }, ref) {
  return (
    ...
    >
      ...
      >
        <path
          className="bp4-popover-arrow-border"
...
const Icon = styled.span.attrs(p => ({
  className: `bp4-icon bp4-icon-${p.iconName}`,
}))`
  margin-left: -24px;
  margin-right: 8px;
`
...
const Button = styled.button.attrs(p => ({
  className: `bp4-button bp4-minimal bp4-icon-tick bp4-intent-${
    INTENTS[p.persona]
  }`,
