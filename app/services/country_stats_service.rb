...
class CountryStatsService
  ...
  def self.format_country_stats(raw_stats)
    ...
    visit_counts = merged_stats.map { |r| r[:unique_visits] || 0 }

    {
      stats: merged_stats.sort_by { |s| -s[:unique_visits] },
      min_visits: visit_counts.min || 0,
      max_visits: visit_counts.max || 0,
      total_visits: merged_stats.sum { |s| s[:unique_visits] || 0 },
      country_count: merged_stats.size,
      total_deployments: merged_stats.sum { |s| s[:deployments_count] },
      total_podcast_listens: merged_stats.sum { |s| s[:visit_podcast_count] }
    }
  end
end
