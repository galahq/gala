...
| `BASE_URL` | generated-by-SST | SST stage/domain | yes | yes | yes | `https://staging.learngala.com` or `https://www.learngala.com` by stage. |
...
- Keep `DATABASE_URL`, `REDIS_URL`, `BASE_URL`, `AWS_REGION`, and `S3_BUCKET` generated by SST unless a migration rehearsal explicitly needs an override.
