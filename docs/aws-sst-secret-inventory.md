...
| `CLOUDFLARE_ACCOUNT_ID` | DNS automation secret | SST secret | no | no | no | Used by non-production operator/DNS automation for the Cloudflare account that owns `learngala.dev`; not injected into app containers. |
| `CLOUDFLARE_API_TOKEN` | DNS automation secret | SST secret | no | no | no | Non-production Cloudflare API token with access to the `learngala.dev` zone; not injected into app containers. |
...
- Load Cloudflare credentials from local operator environment files into non-production SST stages with `bin/cfver --sync-sst-secrets`; keep those credentials out of Rails container environment unless a future DNS automation task explicitly needs them.
