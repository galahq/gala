...
index 597d6c89..e969abb9 100644
--- a/.dockerignore
+++ b/.dockerignore
@@ -12,4 +12,4 @@ config/master.key
...
index 8dfc3363..e0fe14df 100644
--- a/.gitignore
+++ b/.gitignore
@@ -53,4 +53,8 @@ spec/examples.txt
 .env.dev
...
index c4a602db..34cde569 100644
--- a/.ruby-version
+++ b/.ruby-version
@@ -1 +1 @@
-3.2.6
...
+
   ...
   &__sidebar
     margin: 0 1em
diff --git a/app/controllers/cases/stats_controller.rb b/app/controllers/cases/stats_controller.rb
index cb3ed599..d96c90d9 100644
--- a/app/controllers/cases/stats_controller.rb
+++ b/app/controllers/cases/stats_controller.rb
@@ -3,7 +3,7 @@
 module Cases
...
+    # Build map data for the stats page. Returns country counts and visit
+    # coordinates filtered by the provided date range.
+    def map_data
+      canned = params[:query]
+
+      # If a canned query other than default is requested, delegate to StatsService
+      if canned.present? && canned != 'case'
+        service = StatsService.new(case_id: @case.id,
+                                   canned_id: canned,
+                                   from: params[:from],
+                                   to: params[:to])
+        return service.run
...
+        from_time = from_param.present? ? Time.zone.parse(from_param) : nil
+        to_time = to_param.present? ? Time.zone.parse(to_param) : nil
+        # Inclusive range on both ends when both present
+        range = if from_time && to_time
+                  from_time.beginning_of_day..to_time.end_of_day
+                elsif from_time
+                  from_time.beginning_of_day..Time.zone.now
+                elsif to_time
+                  Time.at(0)..to_time.end_of_day
+                end
+      end
+
+      events = Ahoy::Event.interesting
+                          .for_case(@case)
+                          .where(name: 'visit_element')
+      events = events.where(time: range) if range
+
+      # Country counts by distinct visit id (session) per country
+      country_counts =
+        events.joins(:visit)
+              .group("COALESCE(visits.country, 'Unknown')")
+              .count('DISTINCT visits.id')
+
+      # A capped set of visit coordinates for heatmap display
+      point_rows =
+        events.joins(:visit)
+              .where.not('visits.latitude' => nil)
+              .where.not('visits.longitude' => nil)
+              .select([
+                'DISTINCT visits.id',
+                'visits.latitude',
+                'visits.longitude',
+                "COALESCE(visits.country, 'Unknown') AS country"
+              ].join(', '))
+              .limit(5000)
+
+      # Centroid per country for a simple aggregated visualization
+      centroid_rows =
+        events.joins(:visit)
+              .where.not('visits.country' => nil)
+              .where.not('visits.latitude' => nil)
+              .where.not('visits.longitude' => nil)
+              .group("COALESCE(visits.country, 'Unknown')")
+              .select([
+                "COALESCE(visits.country, 'Unknown') AS country",
+                'COUNT(DISTINCT visits.id) AS count',
+                'AVG(CAST(visits.latitude AS double precision)) AS lat',
+                'AVG(CAST(visits.longitude AS double precision)) AS lon'
+              ].join(', '))
+
+      {
+        countryCounts: country_counts.map do |country, count|
+          { country: country, count: count }
+        end,
+        points: point_rows.map do |v|
+                  {
+                    type: 'Feature',
+                    properties: { country: v.country },
+                    geometry: {
+                      type: 'Point',
+                      coordinates: [v.longitude.to_f, v.latitude.to_f]
+                    }
+                  }
+                end,
+        centroids: centroid_rows.map do |row|
+          {
+            type: 'Feature',
+            properties: {
+              country: row.country,
...
+            geometry: {
+              type: 'Point',
+              coordinates: [row.lon.to_f, row.lat.to_f]
+            }
+          }
+        end
+      }
...
+
+    def set_stats
+      @stats = Ahoy::Event.find_by_sql(<<~SQL)
+        SELECT
+          interesting_readers.locale AS reader_locale,
+          COUNT(DISTINCT interesting_readers.reader_id) AS user_count_all_time,
+          COUNT(DISTINCT interesting_readers.reader_id) FILTER (WHERE ahoy_events.time >= NOW() - INTERVAL '1 year') AS user_count_last_1year,
+          COUNT(DISTINCT interesting_readers.reader_id) FILTER (WHERE ahoy_events.time >= NOW() - INTERVAL '2 years') AS user_count_last_2years,
+          COUNT(DISTINCT visits.visitor_token) AS session_count_all_time,
+          COUNT(DISTINCT visits.visitor_token) FILTER (WHERE ahoy_events.time >= NOW() - INTERVAL '1 year') AS session_count_last_1year,
+          COUNT(DISTINCT visits.visitor_token) FILTER (WHERE ahoy_events.time >= NOW() - INTERVAL '2 years') AS session_count_last_2years,
...
+          DATE(MAX(ahoy_events.time)) AS visit_date_most_recent,
+          DATE(MIN(ahoy_events.time)) AS visit_date_earliest
+        FROM
+          cases
+          INNER JOIN ahoy_events ON cases.slug = ahoy_events.properties ->> 'case_slug'
+          LEFT JOIN visits ON ahoy_events.visit_id = visits.id
+          LEFT JOIN (
+            SELECT
+              readers.id AS reader_id, readers.locale
+            FROM
+              readers
+              LEFT JOIN readers_roles ON readers_roles.reader_id = readers.id
+              LEFT JOIN roles ON roles.id = readers_roles.role_id
+            WHERE
+              roles.name <> 'invisible' OR roles.name IS NULL
+          ) AS interesting_readers
+            ON interesting_readers.reader_id = ahoy_events.user_id
+        WHERE
+          cases.id = #{@case.id}
+        GROUP BY reader_locale
+        ORDER BY
+          user_count_all_time DESC;
+      SQL
+
+      @stats = {
...
@@ -32,11 +184,46 @@ module Cases
       @editorships = @case.editorships
     end
 
+    def set_results_rows
+      @results_rows = [
+        {
+          activity: t('.deployments', default: 'Deployments'),
+          last_7: @case.deployments.where(created_at: 7.days.ago..).size,
+          last_6m: @case.deployments.where(created_at: 6.months.ago..).size,
+          last_12m: @case.deployments.where(created_at: 12.months.ago..).size,
+          all_time: @case.deployments.size
+        },
+        {
+          activity: t('.visits', default: 'Visits'),
+          last_7: @case.events.interesting.where_properties(name: 'visit_element').where(time: 7.days.ago..).group(:user_id).count.keys.count,
+          last_6m: @case.events.interesting.where_properties(name: 'visit_element').where(time: 6.months.ago..).group(:user_id).count.keys.count,
+          last_12m: @case.events.interesting.where_properties(name: 'visit_element').where(time: 12.months.ago..).group(:user_id).count.keys.count,
+          all_time: @case.events.interesting.where_properties(name: 'visit_element').group(:user_id).count.keys.count
+        },
+        {
+          activity: t('.locales', default: 'Locales'),
+          last_7: helpers.locales_to_sentence(Reader.where(id: @case.events.interesting.where_properties(name: 'visit_element').where(time: 7.days.ago..).group(:user_id).count.keys).pluck(:locale)),
+          last_6m: helpers.locales_to_sentence(Reader.where(id: @case.events.interesting.where_properties(name: 'visit_element').where(time: 6.months.ago..).group(:user_id).count.keys).pluck(:locale)),
+          last_12m: helpers.locales_to_sentence(Reader.where(id:  @case.events.interesting.where_properties(name: 'visit_element').where(time: 12.months.ago..).group(:user_id).count.keys).pluck(:locale)),
+          all_time: helpers.locales_to_sentence(Reader.where(id:  @case.events.interesting.where_properties(name: 'visit_element').group(:user_id).count.keys).pluck(:locale))
+        }
+      ]
+    end
+
+    def set_headline_stats
+      events = Ahoy::Event.interesting.for_case(@case).where(name: 'visit_element')
+      @headline_stats = {
+        t('.unique_visitors', default: 'Unique visitors') => events.distinct.count(:user_id),
+        t('.sessions', default: 'Sessions') => events.distinct.count(:visit_id),
+        t('.latest_visit', default: 'Latest visit') => events.maximum(:time)&.to_date || '-',
+        t('.earliest_visit', default: 'Earliest visit') => events.minimum(:time)&.to_date || '-'
+      }
+    end
+
...
diff --git a/app/javascript/shared/blueprint.scss b/app/javascript/shared/blueprint.scss
index 24f92378..5cbc4c31 100644
--- a/app/javascript/shared/blueprint.scss
+++ b/app/javascript/shared/blueprint.scss
@@ -35,6 +35,8 @@ $green5: hsl(125, 48%, 61%);
...
+@import '~@blueprintjs/table/src/table';
 ...
 h1, h2, h3, h4, h5, h6 {
   line-height: unset;
diff --git a/app/views/cases/stats/show.html.haml.bak b/app/views/cases/stats/show.html.haml.bak
deleted file mode 100644
index b3c08ea8..00000000
--- a/app/views/cases/stats/show.html.haml.bak
+++ /dev/null
@@ -1,66 +0,0 @@
...
index 0e3ddb40..050d17be 100644
...
+    restart: always
+    healthcheck:
+      test: ["CMD-SHELL", "pg_isready -U gala"]
+      interval: 10s
+      timeout: 5s
+      retries: 5
     environment:
...
index 5cd74590..b7f47dbe 100644
--- a/package.json
+++ b/package.json
@@ -29,6 +29,8 @@
...
index a3e8617c..512b0283 100644
--- a/yarn.lock
+++ b/yarn.lock
@@ -4,7 +4,7 @@
...
-"@babel/core@>=7.2.2", "@babel/core@^7.1.0", "@babel/core@^7.2.0", "@babel/core@^7.7.2":
+"@babel/core@^7.0.0", "@babel/core@^7.0.0-0", "@babel/core@^7.1.0", "@babel/core@^7.2.0", "@babel/core@^7.7.2", "@babel/core@>=7.2.2":
   version "7.7.2"
...
@@ -272,7 +272,7 @@
 ...
 "@babel/plugin-proposal-class-properties@^7.2.1", "@babel/plugin-proposal-class-properties@^7.7.0":
   version "7.7.0"
...
@@ -693,7 +693,7 @@
 ...
 "@babel/preset-env@^7.2.0", "@babel/preset-env@^7.7.1":
   version "7.7.1"
...
@@ -777,14 +777,14 @@
 ...
 "@babel/runtime@^7.0.0", "@babel/runtime@^7.1.2", "@babel/runtime@^7.2.0", "@babel/runtime@^7.3.1", "@babel/runtime@^7.4.2", "@babel/runtime@^7.4.3", "@babel/runtime@^7.4.5", "@babel/runtime@^7.6.3", "@babel/runtime@^7.7.2":
   version "7.7.2"
...
+  resolved "https://registry.npmjs.org/@blueprintjs/core/-/core-2.3.1.tgz"
   integrity sha512-huOi4cyICMZb3YwbLgsq/sKWTvIAJw9RZrmarNvVXxK7XJLIk8xSyVZWxJchf+ESpVWskBhZS1cT9b736KptsQ==
   dependencies:
...
+"@blueprintjs/datetime@^2.0.3":
+  version "2.0.3"
+  resolved "https://registry.npmjs.org/@blueprintjs/datetime/-/datetime-2.0.3.tgz"
+  integrity sha512-LUxXdYsBrl+mCzqpT1y/QHvAfafqPydsQ8A6YhzipEi3gEEWTChvsqfwC7hq8lgoXiR4FkRVMg/dNQOkDCdDIw==
+  dependencies:
+    "@blueprintjs/core" "^2.3.1"
+    classnames "^2.2"
...
+  resolved "https://registry.npmjs.org/@blueprintjs/icons/-/icons-2.2.1.tgz"
   integrity sha512-aMFjwfI6eEliw+Rqyhzt0ZbetxQgKO+fsnWefMqUEYG9eix/PSNPD8Bb/v1vyaFSO1Doa74G5Xj6d5AgNZt+Ww==
   dependencies:
     classnames "^2.2"
@@ -839,16 +849,26 @@
 ...
 "@blueprintjs/select@^2.0.1":
   version "2.0.1"
-  resolved "https://registry.yarnpkg.com/@blueprintjs/select/-/select-2.0.1.tgz#4746aff7ed534113b17d0663bf548b2867dc4b2a"
+  resolved "https://registry.npmjs.org/@blueprintjs/select/-/select-2.0.1.tgz"
   integrity sha512-QTNzYrc6LeBxNTAGr4wqFCjf5AXfNC8XfYt/H+rPmNJyDusVutDID4IXzhFlMM+Xt8xDNu7ayqKO6H3S82fjfw==
   dependencies:
     "@blueprintjs/core" "^2.3.1"
     classnames "^2.2"
     tslib "^1.9.0"
...
+"@blueprintjs/table@^2.1.1":
+  version "2.1.1"
+  resolved "https://registry.npmjs.org/@blueprintjs/table/-/table-2.1.1.tgz"
+  integrity sha512-PPqGlgC+dZ6K9TN9NY4kfOBgkdgiTYeP7EWc4M+4tOBjPCqQOPrDc452viVUqNa5B0opv/WkySpr9yxO1fFyng==
+  dependencies:
+    "@blueprintjs/core" "^2.3.1"
+    classnames "^2.2"
+    prop-types "^15.6.0"
...
+  resolved "https://registry.npmjs.org/@rails/webpacker/-/webpacker-4.2.0.tgz"
...
+  resolved "https://registry.npmjs.org/accepts/-/accepts-1.3.7.tgz"
   ...
   dependencies:
     mime-types "~2.1.24"
@@ -1475,7 +1495,7 @@ accepts@~1.3.4, accepts@~1.3.5, accepts@~1.3.7:
...
+  resolved "https://registry.npmjs.org/acorn-globals/-/acorn-globals-4.3.4.tgz"
   ...
   dependencies:
     acorn "^6.0.1"
@@ -1483,34 +1503,34 @@ acorn-globals@^4.1.0:
...
+"acorn@^6.0.0 || ^7.0.0", acorn@^6.0.1, acorn@^6.0.7, acorn@^6.2.1:
   version "6.3.0"
...
+  resolved "https://registry.npmjs.org/acorn/-/acorn-6.3.0.tgz"
 ...
 activestorage@^5.2.0, activestorage@^5.2.1:
   version "5.2.3"
-  resolved "https://registry.yarnpkg.com/activestorage/-/activestorage-5.2.3.tgz#04df0a194d15f7a04abbba313f584fbea2c5a66f"
+  resolved "https://registry.npmjs.org/activestorage/-/activestorage-5.2.3.tgz"
   integrity sha512-B0PHne9Bvpsapw1WqPZF8YafdN5pohOAUgR5ZGSijBOJKDGb+IJ6ib8Js9mfWNCmuCPCrCzHca/mJ2wD7UBJxA==
   dependencies:
     spark-md5 "^3.0.0"
...
+  resolved "https://registry.npmjs.org/aggregate-error/-/aggregate-error-3.0.1.tgz"
   ...
   dependencies:
     clean-stack "^2.0.0"
@@ -1518,17 +1538,17 @@ aggregate-error@^3.0.0:
...
+ajv@^6.1.0, ajv@^6.10.2, ajv@^6.5.5, ajv@^6.9.1, ajv@>=5.0.0:
   version "6.10.2"
...
@@ -1538,69 +1558,74 @@ ajv@^6.1.0, ajv@^6.10.2, ajv@^6.5.5, ajv@^6.9.1:
...
+  resolved "https://registry.npmjs.org/anymatch/-/anymatch-2.0.0.tgz"
   ...
   dependencies:
     micromatch "^3.1.4"
@@ -1608,12 +1633,12 @@ anymatch@^2.0.0:
...
+  resolved "https://registry.npmjs.org/are-we-there-yet/-/are-we-there-yet-1.1.5.tgz"
   ...
   dependencies:
     delegates "^1.0.0"
@@ -1621,14 +1646,14 @@ are-we-there-yet@~1.1.2:
...
@@ -1644,42 +1669,42 @@ aria-query@^3.0.0:
...
@@ -1687,34 +1712,34 @@ array-includes@^3.0.3:
...
+  resolved "https://registry.npmjs.org/autoprefixer/-/autoprefixer-7.2.6.tgz"
   ...
   dependencies:
     browserslist "^2.11.3"
@@ -1807,7 +1832,7 @@ autoprefixer@^7.1.1:
 
 autoprefixer@^9.0.0, autoprefixer@^9.4.2, autoprefixer@^9.6.1:
   version "9.7.2"
...
+  resolved "https://registry.npmjs.org/autoprefixer/-/autoprefixer-9.7.2.tgz"
   ...
   dependencies:
     browserslist "^4.7.3"
@@ -1820,24 +1845,24 @@ autoprefixer@^9.0.0, autoprefixer@^9.4.2, autoprefixer@^9.6.1:
...
+  resolved "https://registry.npmjs.org/babel-eslint/-/babel-eslint-10.0.3.tgz"
   ...
   dependencies:
     "@babel/code-frame" "^7.0.0"
@@ -1849,7 +1874,7 @@ babel-eslint@^10.0.1:
...
+  resolved "https://registry.npmjs.org/babel-jest/-/babel-jest-24.9.0.tgz"
   ...
   dependencies:
     "@jest/transform" "^24.9.0"
@@ -1862,7 +1887,7 @@ babel-jest@^24.5.0, babel-jest@^24.9.0:
...
@@ -1872,14 +1897,14 @@ babel-loader@^8.0.0, babel-loader@^8.0.6:
...
+  resolved "https://registry.npmjs.org/babel-plugin-istanbul/-/babel-plugin-istanbul-5.2.0.tgz"
   ...
   dependencies:
     "@babel/helper-plugin-utils" "^7.0.0"
@@ -1889,23 +1914,23 @@ babel-plugin-istanbul@^5.1.0:
...
-"babel-plugin-styled-components@>= 1", babel-plugin-styled-components@^1.9.4:
+babel-plugin-styled-components@^1.9.4, "babel-plugin-styled-components@>= 1":
   version "1.10.6"
...
+  resolved "https://registry.npmjs.org/babel-plugin-styled-components/-/babel-plugin-styled-components-1.10.6.tgz"
   ...
   dependencies:
     "@babel/helper-annotate-as-pure" "^7.0.0"
@@ -1915,12 +1940,12 @@ babel-plugin-macros@^2.6.1:
...
@@ -1928,7 +1953,7 @@ babel-preset-jest@^24.9.0:
...
+  resolved "https://registry.npmjs.org/babel-runtime/-/babel-runtime-6.26.0.tgz"
   ...
   dependencies:
     core-js "^2.4.0"
@@ -1936,32 +1961,27 @@ babel-runtime@^6.11.6, babel-runtime@^6.23.0, babel-runtime@^6.26.0:
...
+  resolved "https://registry.npmjs.org/body-parser/-/body-parser-1.19.0.tgz"
   ...
   dependencies:
     bytes "3.1.0"
@@ -2044,7 +2069,7 @@ body-parser@1.19.0:
...
+  resolved "https://registry.npmjs.org/brace-expansion/-/brace-expansion-1.1.11.tgz"
   ...
   dependencies:
     balanced-match "^1.0.0"
@@ -2069,7 +2094,7 @@ brace-expansion@^1.1.7:
...
+  resolved "https://registry.npmjs.org/braces/-/braces-2.3.2.tgz"
   ...
   dependencies:
     arr-flatten "^1.1.0"
@@ -2085,24 +2110,24 @@ braces@^2.3.1, braces@^2.3.2:
...
+  resolved "https://registry.npmjs.org/browserify-aes/-/browserify-aes-1.2.0.tgz"
   ...
   dependencies:
     buffer-xor "^1.0.3"
@@ -2114,7 +2139,7 @@ browserify-aes@^1.0.0, browserify-aes@^1.0.4:
...
@@ -2123,7 +2148,7 @@ browserify-cipher@^1.0.0:
...
+  resolved "https://registry.npmjs.org/browserify-des/-/browserify-des-1.0.2.tgz"
   ...
   dependencies:
     cipher-base "^1.0.1"
@@ -2133,7 +2158,7 @@ browserify-des@^1.0.0:
...
+  resolved "https://registry.npmjs.org/browserify-rsa/-/browserify-rsa-4.0.1.tgz"
   ...
   dependencies:
     bn.js "^4.1.0"
@@ -2141,7 +2166,7 @@ browserify-rsa@^4.0.0:
...
+  resolved "https://registry.npmjs.org/browserify-sign/-/browserify-sign-4.0.4.tgz"
   ...
   dependencies:
     bn.js "^4.1.1"
@@ -2154,14 +2179,14 @@ browserify-sign@^4.0.0:
...
@@ -2169,7 +2194,7 @@ browserslist@^2.0.0, browserslist@^2.11.3:
 
 browserslist@^4.0.0, browserslist@^4.6.0, browserslist@^4.6.4, browserslist@^4.7.2, browserslist@^4.7.3:
   version "4.7.3"
...
@@ -2178,29 +2203,29 @@ browserslist@^4.0.0, browserslist@^4.6.0, browserslist@^4.6.4, browserslist@^4.7
...
@@ -2304,36 +2329,36 @@ cache-base@^1.0.1:
...
+  resolved "https://registry.npmjs.org/camel-case/-/camel-case-3.0.0.tgz"
   ...
   dependencies:
     no-case "^2.2.0"
@@ -2341,7 +2366,7 @@ camel-case@^3.0.0:
...
+  resolved "https://registry.npmjs.org/camelcase-keys/-/camelcase-keys-2.1.0.tgz"
   ...
   dependencies:
     camelcase "^2.0.0"
@@ -2349,7 +2374,7 @@ camelcase-keys@^2.0.0:
...
+  resolved "https://registry.npmjs.org/camelcase-keys/-/camelcase-keys-4.2.0.tgz"
   ...
   dependencies:
     camelcase "^4.1.0"
@@ -2358,32 +2383,32 @@ camelcase-keys@^4.0.0:
...
+  resolved "https://registry.npmjs.org/caniuse-api/-/caniuse-api-2.0.0.tgz"
   ...
   dependencies:
     browserslist "^2.0.0"
@@ -2393,7 +2418,7 @@ caniuse-api@^2.0.0:
...
+  resolved "https://registry.npmjs.org/caniuse-api/-/caniuse-api-3.0.0.tgz"
   ...
   dependencies:
     browserslist "^4.0.0"
@@ -2401,21 +2426,21 @@ caniuse-api@^3.0.0:
     lodash.memoize "^4.1.2"
     lodash.uniq "^4.5.0"
 
-caniuse-lite@^1.0.0, caniuse-lite@^1.0.30000792, caniuse-lite@^1.0.30000805, caniuse-lite@^1.0.30000921, caniuse-lite@^1.0.30000981, caniuse-lite@^1.0.30001010:
+caniuse-lite@^1.0.0, caniuse-lite@^1.0.30000697, caniuse-lite@^1.0.30000792, caniuse-lite@^1.0.30000805, caniuse-lite@^1.0.30000921, caniuse-lite@^1.0.30000981, caniuse-lite@^1.0.30001010:
...
+  resolved "https://registry.npmjs.org/cardinal/-/cardinal-0.4.4.tgz"
   ...
   dependencies:
     ansicolors "~0.2.1"
@@ -2423,31 +2448,33 @@ cardinal@~0.4.2:
...
+  resolved "https://registry.npmjs.org/chalk/-/chalk-1.1.3.tgz"
   ...
   dependencies:
     ansi-styles "^2.2.1"
@@ -2456,9 +2483,18 @@ chalk@^1.1.1, chalk@^1.1.3:
     strip-ansi "^3.0.0"
     supports-color "^2.0.0"
 
+chalk@^2.0, chalk@^2.0.0, chalk@^2.0.1, chalk@^2.1.0, chalk@^2.4.1, chalk@^2.4.2, chalk@2.4.2:
+  version "2.4.2"
...
+  resolved "https://registry.npmjs.org/change-case/-/change-case-3.1.0.tgz"
   ...
   dependencies:
     camel-case "^3.0.0"
@@ -2482,42 +2518,42 @@ change-case@^3.0.2:
...
+  resolved "https://registry.npmjs.org/chokidar/-/chokidar-2.1.8.tgz"
   ...
   dependencies:
     anymatch "^2.0.0"
@@ -2536,24 +2572,24 @@ chokidar@^2.0.2, chokidar@^2.1.8:
...
+  resolved "https://registry.npmjs.org/cipher-base/-/cipher-base-1.0.4.tgz"
   ...
   dependencies:
     inherits "^2.0.1"
@@ -2561,7 +2597,7 @@ cipher-base@^1.0.0, cipher-base@^1.0.1, cipher-base@^1.0.3:
...
+  resolved "https://registry.npmjs.org/class-utils/-/class-utils-0.3.6.tgz"
   ...
   dependencies:
     arr-union "^3.1.0"
@@ -2571,29 +2607,29 @@ class-utils@^0.3.5:
...
@@ -2629,7 +2665,7 @@ clone-deep@^4.0.1:
...
+  resolved "https://registry.npmjs.org/clone-regexp/-/clone-regexp-1.0.1.tgz"
   ...
   dependencies:
     is-regexp "^1.0.0"
@@ -2637,22 +2673,22 @@ clone-regexp@^1.0.0:
...
+  resolved "https://registry.npmjs.org/collection-visit/-/collection-visit-1.0.0.tgz"
   ...
   dependencies:
     map-visit "^1.0.0"
@@ -2684,36 +2720,36 @@ collection-visit@^1.0.0:
...
+  resolved "https://registry.npmjs.org/color-string/-/color-string-1.5.3.tgz"
   ...
   dependencies:
     color-name "^1.0.0"
@@ -2721,7 +2757,7 @@ color-string@^1.4.0, color-string@^1.5.2:
...
+  resolved "https://registry.npmjs.org/combined-stream/-/combined-stream-1.0.8.tgz"
 ...
 commander@^2.11.0, commander@^2.18.0, commander@^2.20.0, commander@^2.8.1, commander@~2.20.3:
   version "2.20.3"
...
+  resolved "https://registry.npmjs.org/compression-webpack-plugin/-/compression-webpack-plugin-3.0.0.tgz"
   ...
   dependencies:
     cacache "^11.2.0"
@@ -2795,7 +2831,7 @@ compression-webpack-plugin@^3.0.0:
...
+  resolved "https://registry.npmjs.org/compression/-/compression-1.7.4.tgz"
   ...
   dependencies:
     accepts "~1.3.5"
@@ -2808,12 +2844,12 @@ compression@^1.7.4:
...
+  resolved "https://registry.npmjs.org/concat-stream/-/concat-stream-1.6.2.tgz"
   ...
   dependencies:
     buffer-from "^1.0.0"
@@ -2823,22 +2859,22 @@ concat-stream@^1.5.0, concat-stream@~1.6.0:
...
+  resolved "https://registry.npmjs.org/constant-case/-/constant-case-2.0.0.tgz"
   ...
   dependencies:
     snake-case "^2.1.0"
@@ -2846,46 +2882,46 @@ constant-case@^2.0.0:
...
+  resolved "https://registry.npmjs.org/copy-concurrently/-/copy-concurrently-1.0.5.tgz"
   ...
   dependencies:
     aproba "^1.1.1"
@@ -2897,12 +2933,12 @@ copy-concurrently@^1.0.0:
...
+  resolved "https://registry.npmjs.org/core-js-compat/-/core-js-compat-3.4.1.tgz"
   ...
   dependencies:
     browserslist "^4.7.2"
@@ -2910,27 +2946,27 @@ core-js-compat@^3.1.1:
...
+  resolved "https://registry.npmjs.org/cosmiconfig/-/cosmiconfig-5.2.1.tgz"
   ...
   dependencies:
     import-fresh "^2.0.0"
@@ -2940,7 +2976,7 @@ cosmiconfig@^5.0.0:
...
+  resolved "https://registry.npmjs.org/cosmiconfig/-/cosmiconfig-6.0.0.tgz"
   ...
   dependencies:
     "@types/parse-json" "^4.0.0"
@@ -2951,7 +2987,7 @@ cosmiconfig@^6.0.0:
...
+  resolved "https://registry.npmjs.org/create-ecdh/-/create-ecdh-4.0.3.tgz"
   ...
   dependencies:
     bn.js "^4.1.0"
@@ -2959,7 +2995,7 @@ create-ecdh@^4.0.0:
...
+  resolved "https://registry.npmjs.org/create-hash/-/create-hash-1.2.0.tgz"
   ...
   dependencies:
     cipher-base "^1.0.1"
@@ -2970,7 +3006,7 @@ create-hash@^1.1.0, create-hash@^1.1.2:
...
+  resolved "https://registry.npmjs.org/create-hmac/-/create-hmac-1.1.7.tgz"
   ...
   dependencies:
     cipher-base "^1.0.3"
@@ -2982,7 +3018,7 @@ create-hmac@^1.1.0, create-hmac@^1.1.2, create-hmac@^1.1.4:
...
+  resolved "https://registry.npmjs.org/create-react-context/-/create-react-context-0.3.0.tgz"
   ...
   dependencies:
     gud "^1.0.0"
@@ -2990,15 +3026,23 @@ create-react-context@^0.3.0:
...
+  resolved "https://registry.npmjs.org/cross-spawn/-/cross-spawn-6.0.5.tgz"
   ...
   dependencies:
     nice-try "^1.0.4"
@@ -3007,17 +3051,9 @@ cross-spawn@6.0.5, cross-spawn@^6.0.0, cross-spawn@^6.0.5:
...
+  resolved "https://registry.npmjs.org/crypto-browserify/-/crypto-browserify-3.12.0.tgz"
   ...
   dependencies:
     browserify-cipher "^1.0.0"
@@ -3034,21 +3070,21 @@ crypto-browserify@^3.11.0:
...
+  resolved "https://registry.npmjs.org/css-color-function/-/css-color-function-1.3.3.tgz"
   ...
   dependencies:
     balanced-match "0.1.0"
@@ -3058,17 +3094,17 @@ css-color-function@~1.3.3:
...
+  resolved "https://registry.npmjs.org/css-declaration-sorter/-/css-declaration-sorter-4.0.1.tgz"
   ...
   dependencies:
     postcss "^7.0.1"
@@ -3076,7 +3112,7 @@ css-declaration-sorter@^4.0.1:
...
+  resolved "https://registry.npmjs.org/css-has-pseudo/-/css-has-pseudo-0.10.0.tgz"
   ...
   dependencies:
     postcss "^7.0.6"
@@ -3084,7 +3120,7 @@ css-has-pseudo@^0.10.0:
...
+  resolved "https://registry.npmjs.org/css-loader/-/css-loader-2.1.1.tgz"
   ...
   dependencies:
     camelcase "^5.2.0"
@@ -3101,7 +3137,7 @@ css-loader@^2.0.1:
...
+  resolved "https://registry.npmjs.org/css-loader/-/css-loader-3.2.0.tgz"
   ...
   dependencies:
     camelcase "^5.3.1"
@@ -3119,19 +3155,19 @@ css-loader@^3.2.0:
...
+  resolved "https://registry.npmjs.org/css-select/-/css-select-2.1.0.tgz"
   ...
   dependencies:
     boolbase "^1.0.0"
@@ -3141,7 +3177,7 @@ css-select@^2.0.0:
...
+  resolved "https://registry.npmjs.org/css-to-react-native/-/css-to-react-native-2.3.2.tgz"
   ...
   dependencies:
     camelize "^1.0.0"
@@ -3150,7 +3186,7 @@ css-to-react-native@^2.2.2:
...
+  resolved "https://registry.npmjs.org/css-tree/-/css-tree-1.0.0-alpha.37.tgz"
   ...
   dependencies:
     mdn-data "2.0.4"
@@ -3158,22 +3194,22 @@ css-tree@1.0.0-alpha.37:
...
+  resolved "https://registry.npmjs.org/cssnano-preset-default/-/cssnano-preset-default-4.0.7.tgz"
   ...
   dependencies:
     css-declaration-sorter "^4.0.1"
@@ -3239,29 +3275,29 @@ cssnano-preset-default@^4.0.7:
...
+  resolved "https://registry.npmjs.org/cssnano/-/cssnano-4.1.10.tgz"
   ...
   dependencies:
     cosmiconfig "^5.0.0"
@@ -3271,80 +3307,122 @@ cssnano@^4.1.10:
...
+  resolved "https://registry.npmjs.org/decamelize-keys/-/decamelize-keys-1.1.0.tgz"
   ...
   dependencies:
     decamelize "^1.1.0"
@@ -3352,27 +3430,27 @@ decamelize-keys@^1.0.0:
...
+  resolved "https://registry.npmjs.org/deep-equal/-/deep-equal-1.1.1.tgz"
   ...
   dependencies:
     is-arguments "^1.0.4"
@@ -3384,17 +3462,17 @@ deep-equal@^1.0.1:
...
+  resolved "https://registry.npmjs.org/default-gateway/-/default-gateway-4.2.0.tgz"
   ...
   dependencies:
     execa "^1.0.0"
@@ -3402,28 +3480,28 @@ default-gateway@^4.2.0:
...
+  resolved "https://registry.npmjs.org/define-property/-/define-property-2.0.2.tgz"
   ...
   dependencies:
     is-descriptor "^1.0.2"
@@ -3431,7 +3509,7 @@ define-property@^2.0.2:
...
+  resolved "https://registry.npmjs.org/diffie-hellman/-/diffie-hellman-5.0.3.tgz"
   ...
   dependencies:
     bn.js "^4.1.0"
@@ -3506,19 +3584,19 @@ diffie-hellman@^5.0.0:
...
+  resolved "https://registry.npmjs.org/dns-packet/-/dns-packet-1.3.1.tgz"
   ...
   dependencies:
     ip "^1.1.0"
@@ -3526,43 +3604,43 @@ dns-packet@^1.3.1:
...
+dom-serializer@^0.2.1, dom-serializer@0:
   version "0.2.2"
...
@@ -3570,7 +3648,7 @@ dom-serializer@0, dom-serializer@^0.2.1:
...
+  resolved "https://registry.npmjs.org/dom-testing-library/-/dom-testing-library-3.19.4.tgz"
   ...
   dependencies:
     "@babel/runtime" "^7.4.3"
@@ -3580,48 +3658,48 @@ dom-testing-library@^3.19.0:
...
+  resolved "https://registry.npmjs.org/domutils/-/domutils-1.7.0.tgz"
   ...
   dependencies:
     dom-serializer "0"
@@ -3629,7 +3707,7 @@ domutils@^1.5.1, domutils@^1.7.0:
...
@@ -3638,21 +3716,21 @@ domutils@^2.0.0:
...
+  resolved "https://registry.npmjs.org/dot-prop/-/dot-prop-4.2.0.tgz"
 ...
 draft-js-linkify-plugin@^2.0.1:
   version "2.0.2"
-  resolved "https://registry.yarnpkg.com/draft-js-linkify-plugin/-/draft-js-linkify-plugin-2.0.2.tgz#0ef40581f0a97728a49f6b010057c6191aad5d45"
+  resolved "https://registry.npmjs.org/draft-js-linkify-plugin/-/draft-js-linkify-plugin-2.0.2.tgz"
   integrity sha512-bW2XwNonPNEbfp9T5PmnUIVLVrX5KpU0OHC5KWmo0B8nnCJlbccvTj6CYL7cCGynTg0wycKbXYuJVdggRrExBg==
   dependencies:
     clsx "^1.0.4"
@@ -3661,7 +3739,7 @@ draft-js-linkify-plugin@^2.0.1:
...
+  resolved "https://registry.npmjs.org/draft-js-plugins-editor/-/draft-js-plugins-editor-2.1.1.tgz"
   ...
   dependencies:
     decorate-component-with-props "^1.0.2"
@@ -3670,9 +3748,9 @@ draft-js-plugins-editor@^2.1.1:
     prop-types "^15.5.8"
     union-class-names "^1.0.0"
 
-draft-js@0.10.5:
+draft-js@^0.10.1, "draft-js@^0.10.1 || ^0.11.0", draft-js@^0.10.5, draft-js@0.10.5:
   version "0.10.5"
...
+  resolved "https://registry.npmjs.org/draft-js/-/draft-js-0.10.5.tgz"
   ...
   dependencies:
     fbjs "^0.8.15"
@@ -3681,12 +3759,12 @@ draft-js@0.10.5:
...
@@ -3696,12 +3774,12 @@ duplexify@^3.4.2, duplexify@^3.6.0:
...
+  resolved "https://registry.npmjs.org/ecc-jsbn/-/ecc-jsbn-0.1.2.tgz"
   ...
   dependencies:
     jsbn "~0.1.0"
@@ -3709,27 +3787,27 @@ ecc-jsbn@~0.1.1:
...
+  resolved "https://registry.npmjs.org/elliptic/-/elliptic-6.5.1.tgz"
   ...
   dependencies:
     bn.js "^4.4.0"
@@ -3742,78 +3820,78 @@ elliptic@^6.0.0:
...
@@ -3829,7 +3907,7 @@ es-abstract@^1.12.0, es-abstract@^1.15.0, es-abstract@^1.5.1, es-abstract@^1.7.0
...
+  resolved "https://registry.npmjs.org/es-to-primitive/-/es-to-primitive-1.2.1.tgz"
   ...
   dependencies:
     is-callable "^1.1.4"
@@ -3838,17 +3916,17 @@ es-to-primitive@^1.2.0:
...
+  resolved "https://registry.npmjs.org/escodegen/-/escodegen-1.12.0.tgz"
   ...
   dependencies:
     esprima "^3.1.3"
@@ -3860,24 +3938,24 @@ escodegen@^1.9.1:
...
+  resolved "https://registry.npmjs.org/eslint-import-resolver-node/-/eslint-import-resolver-node-0.3.2.tgz"
   ...
   dependencies:
     debug "^2.6.9"
@@ -3885,7 +3963,7 @@ eslint-import-resolver-node@^0.3.2:
...
+  resolved "https://registry.npmjs.org/eslint-module-utils/-/eslint-module-utils-2.4.1.tgz"
   ...
   dependencies:
     debug "^2.6.8"
@@ -3893,14 +3971,14 @@ eslint-module-utils@^2.4.0:
...
+  resolved "https://registry.npmjs.org/eslint-plugin-es/-/eslint-plugin-es-1.4.1.tgz"
   ...
   dependencies:
     eslint-utils "^1.4.2"
@@ -3908,14 +3986,14 @@ eslint-plugin-es@^1.3.1:
...
+  resolved "https://registry.npmjs.org/eslint-plugin-import/-/eslint-plugin-import-2.18.2.tgz"
   ...
   dependencies:
     array-includes "^3.0.3"
@@ -3932,7 +4010,7 @@ eslint-plugin-import@^2.14.0:
...
+  resolved "https://registry.npmjs.org/eslint-plugin-jsx-a11y/-/eslint-plugin-jsx-a11y-6.2.3.tgz"
   ...
   dependencies:
     "@babel/runtime" "^7.4.5"
@@ -3945,9 +4023,9 @@ eslint-plugin-jsx-a11y@^6.1.1:
     has "^1.0.3"
     jsx-ast-utils "^2.2.1"
...
+  resolved "https://registry.npmjs.org/eslint-plugin-node/-/eslint-plugin-node-8.0.1.tgz"
   ...
   dependencies:
     eslint-plugin-es "^1.3.1"
@@ -3957,14 +4035,14 @@ eslint-plugin-node@^8.0.0:
     resolve "^1.8.1"
     semver "^5.5.0"
...
+eslint-plugin-react@^7.11.1, eslint-plugin-react@>=7.11.1, eslint-plugin-react@>=7.6.1:
   version "7.16.0"
...
+  resolved "https://registry.npmjs.org/eslint-plugin-react/-/eslint-plugin-react-7.16.0.tgz"
   ...
   dependencies:
     array-includes "^3.0.3"
@@ -3977,19 +4055,19 @@ eslint-plugin-react@^7.11.1:
     prop-types "^15.7.2"
     resolve "^1.12.0"
...
+  resolved "https://registry.npmjs.org/eslint-scope/-/eslint-scope-4.0.3.tgz"
   ...
   dependencies:
     esrecurse "^4.1.0"
@@ -3997,19 +4075,19 @@ eslint-scope@^4.0.3:
...
-eslint@^5.10.0:
+"eslint@^3 || ^4 || ^5 || ^6", "eslint@^3.0.0 || ^4.0.0 || ^5.0.0 || ^6.0.0", eslint@^5.10.0, "eslint@>= 4.12.1", eslint@>=4.0.0, eslint@>=4.19.1, eslint@>=5.0.0, "eslint@2.x - 6.x":
   version "5.16.0"
...
+  resolved "https://registry.npmjs.org/evp_bytestokey/-/evp_bytestokey-1.0.3.tgz"
   ...
   dependencies:
     md5.js "^1.3.4"
@@ -4134,12 +4212,12 @@ evp_bytestokey@^1.0.0, evp_bytestokey@^1.0.3:
...
+  resolved "https://registry.npmjs.org/expand-brackets/-/expand-brackets-2.1.4.tgz"
   ...
   dependencies:
     debug "^2.3.3"
@@ -4177,14 +4255,14 @@ expand-brackets@^2.1.4:
...
+  resolved "https://registry.npmjs.org/express/-/express-4.17.1.tgz"
   ...
   dependencies:
     accepts "~1.3.7"
@@ -4232,14 +4310,22 @@ express@^4.16.3, express@^4.17.1:
...
+  resolved "https://registry.npmjs.org/extend-shallow/-/extend-shallow-3.0.2.tgz"
   ...
   dependencies:
     assign-symbols "^1.0.0"
@@ -4247,12 +4333,12 @@ extend-shallow@^3.0.0, extend-shallow@^3.0.2:
...
+  resolved "https://registry.npmjs.org/external-editor/-/external-editor-3.1.0.tgz"
   ...
   dependencies:
     chardet "^0.7.0"
@@ -4261,7 +4347,7 @@ external-editor@^3.0.3:
...
+  resolved "https://registry.npmjs.org/fast-glob/-/fast-glob-2.2.7.tgz"
   ...
   dependencies:
     "@mrmlnc/readdir-enhanced" "^2.2.1"
@@ -4302,38 +4388,38 @@ fast-glob@^2.2.6:
...
+  resolved "https://registry.npmjs.org/fbjs/-/fbjs-0.8.17.tgz"
   ...
   dependencies:
     core-js "^1.0.0"
@@ -4346,33 +4432,33 @@ fbjs@^0.8.1, fbjs@^0.8.15:
...
+  resolved "https://registry.npmjs.org/file-loader/-/file-loader-2.0.0.tgz"
   ...
   dependencies:
     loader-utils "^1.0.2"
@@ -4380,7 +4466,7 @@ file-loader@^2.0.0:
...
+  resolved "https://registry.npmjs.org/file-loader/-/file-loader-4.2.0.tgz"
   ...
   dependencies:
     loader-utils "^1.2.3"
@@ -4388,12 +4474,12 @@ file-loader@^4.2.0:
...
+  resolved "https://registry.npmjs.org/fill-range/-/fill-range-4.0.0.tgz"
   ...
   dependencies:
     extend-shallow "^2.0.1"
@@ -4403,7 +4489,7 @@ fill-range@^4.0.0:
...
+  resolved "https://registry.npmjs.org/finalhandler/-/finalhandler-1.1.2.tgz"
   ...
   dependencies:
     debug "2.6.9"
@@ -4416,7 +4502,7 @@ finalhandler@~1.1.2:
...
+  resolved "https://registry.npmjs.org/find-cache-dir/-/find-cache-dir-2.1.0.tgz"
   ...
   dependencies:
     commondir "^1.0.1"
@@ -4425,7 +4511,7 @@ find-cache-dir@^2.0.0, find-cache-dir@^2.1.0:
...
+  resolved "https://registry.npmjs.org/find-cache-dir/-/find-cache-dir-3.1.0.tgz"
   ...
   dependencies:
     commondir "^1.0.1"
@@ -4434,29 +4520,36 @@ find-cache-dir@^3.0.0:
...
+  resolved "https://registry.npmjs.org/findup-sync/-/findup-sync-3.0.0.tgz"
   ...
   dependencies:
     detect-file "^1.0.0"
@@ -4479,7 +4572,7 @@ findup-sync@3.0.0:
...
+  resolved "https://registry.npmjs.org/flat-cache/-/flat-cache-2.0.1.tgz"
   ...
   dependencies:
     flatted "^2.0.0"
@@ -4488,29 +4581,29 @@ flat-cache@^2.0.1:
...
+  resolved "https://registry.npmjs.org/flow-bin/-/flow-bin-0.87.0.tgz"
 ...
 flow-inspect@^1.0.1:
   version "1.0.1"
-  resolved "https://registry.yarnpkg.com/flow-inspect/-/flow-inspect-1.0.1.tgz#991f114f4b38ddbea2dc5b34fc2c2bebe12667bd"
+  resolved "https://registry.npmjs.org/flow-inspect/-/flow-inspect-1.0.1.tgz"
   integrity sha1-mR8RT0s43b6i3Fs0/Cwr6+EmZ70=
   dependencies:
     chalk "^1.1.3"
...
+  resolved "https://registry.npmjs.org/flush-write-stream/-/flush-write-stream-1.1.1.tgz"
   ...
   dependencies:
     inherits "^2.0.3"
@@ -4518,24 +4611,24 @@ flush-write-stream@^1.0.0:
...
+  resolved "https://registry.npmjs.org/form-data/-/form-data-2.3.3.tgz"
   ...
   dependencies:
     asynckit "^0.4.0"
@@ -4544,51 +4637,49 @@ form-data@~2.3.2:
...
+  resolved "https://registry.npmjs.org/fs-write-stream-atomic/-/fs-write-stream-atomic-1.0.10.tgz"
   ...
   dependencies:
     graceful-fs "^4.1.2"
@@ -4598,12 +4689,12 @@ fs-write-stream-atomic@^1.0.8:
...
+  resolved "https://registry.npmjs.org/fsevents/-/fsevents-1.2.9.tgz"
   ...
   dependencies:
     nan "^2.12.1"
@@ -4611,12 +4702,12 @@ fsevents@^1.2.7:
...
+  resolved "https://registry.npmjs.org/fstream/-/fstream-1.0.12.tgz"
   ...
   dependencies:
     graceful-fs "^4.1.2"
@@ -4626,17 +4717,17 @@ fstream@^1.0.0, fstream@^1.0.12:
...
+  resolved "https://registry.npmjs.org/glob-parent/-/glob-parent-3.1.0.tgz"
   ...
   dependencies:
     is-glob "^3.1.0"
@@ -4714,12 +4805,12 @@ glob-parent@^3.1.0:
...
+  resolved "https://registry.npmjs.org/glob/-/glob-7.1.6.tgz"
   ...
   dependencies:
     fs.realpath "^1.0.0"
@@ -4729,25 +4820,25 @@ glob@^7.0.0, glob@^7.0.3, glob@^7.1.1, glob@^7.1.2, glob@^7.1.3, glob@^7.1.4, gl
     once "^1.3.0"
...
+  resolved "https://registry.npmjs.org/global-prefix/-/global-prefix-1.0.2.tgz"
   ...
   dependencies:
     expand-tilde "^2.0.2"
@@ -4758,7 +4849,7 @@ global-prefix@^1.0.1:
...
+  resolved "https://registry.npmjs.org/global-prefix/-/global-prefix-3.0.0.tgz"
   ...
   dependencies:
     ini "^1.3.5"
@@ -4767,12 +4858,12 @@ global-prefix@^3.0.0:
...
+  resolved "https://registry.npmjs.org/gzip-size/-/gzip-size-5.1.1.tgz"
   ...
   dependencies:
     duplexer "^0.1.1"
@@ -4846,17 +4937,17 @@ gzip-size@^5.0.0:
...
+  resolved "https://registry.npmjs.org/handlebars/-/handlebars-4.5.3.tgz"
   ...
   dependencies:
     neo-async "^2.6.0"
@@ -4867,12 +4958,12 @@ handlebars@^4.1.2:
...
+  resolved "https://registry.npmjs.org/har-validator/-/har-validator-5.1.3.tgz"
   ...
   dependencies:
     ajv "^6.5.5"
@@ -4880,29 +4971,29 @@ har-validator@~5.1.0:
...
+  resolved "https://registry.npmjs.org/has-value/-/has-value-1.0.0.tgz"
   ...
   dependencies:
     get-value "^2.0.6"
@@ -4920,12 +5011,12 @@ has-value@^1.0.0:
...
+  resolved "https://registry.npmjs.org/has-values/-/has-values-1.0.0.tgz"
   ...
   dependencies:
     is-number "^3.0.0"
@@ -4933,14 +5024,14 @@ has-values@^1.0.0:
...
+  resolved "https://registry.npmjs.org/hash.js/-/hash.js-1.1.7.tgz"
   ...
   dependencies:
     inherits "^2.0.3"
@@ -4956,7 +5047,7 @@ hash.js@^1.0.0, hash.js@^1.0.3:
...
+  resolved "https://registry.npmjs.org/header-case/-/header-case-1.0.1.tgz"
   ...
   dependencies:
     no-case "^2.2.0"
@@ -4964,12 +5055,12 @@ header-case@^1.0.0:
...
+  resolved "https://registry.npmjs.org/history/-/history-4.10.1.tgz"
   ...
   dependencies:
     "@babel/runtime" "^7.1.2"
@@ -4981,7 +5072,7 @@ history@^4.6.1, history@^4.7.2:
...
+  resolved "https://registry.npmjs.org/hmac-drbg/-/hmac-drbg-1.0.1.tgz"
   ...
   dependencies:
     hash.js "^1.0.3"
@@ -4990,36 +5081,36 @@ hmac-drbg@^1.0.0:
...
+  resolved "https://registry.npmjs.org/hpack.js/-/hpack.js-2.1.6.tgz"
   ...
   dependencies:
     inherits "^2.0.1"
@@ -5029,39 +5120,39 @@ hpack.js@^2.1.6:
...
+  resolved "https://registry.npmjs.org/html-to-react/-/html-to-react-1.4.2.tgz"
   ...
   dependencies:
     domhandler "^3.0"
@@ -5071,7 +5162,7 @@ html-to-react@^1.3.4:
...
@@ -5083,7 +5174,7 @@ htmlparser2@^3.10.0:
...
+  resolved "https://registry.npmjs.org/htmlparser2/-/htmlparser2-4.0.0.tgz"
   ...
   dependencies:
     domelementtype "^2.0.1"
@@ -5093,23 +5184,12 @@ htmlparser2@^4.0:
...
+  resolved "https://registry.npmjs.org/http-errors/-/http-errors-1.6.3.tgz"
   ...
   dependencies:
     depd "~1.1.2"
@@ -5119,7 +5199,7 @@ http-errors@~1.6.2:
...
+  resolved "https://registry.npmjs.org/http-errors/-/http-errors-1.7.3.tgz"
   ...
   dependencies:
     depd "~1.1.2"
@@ -5128,14 +5208,25 @@ http-errors@~1.7.2:
     statuses ">= 1.5.0 < 2"
     toidentifier "1.0.0"
...
+  resolved "https://registry.npmjs.org/http-proxy-middleware/-/http-proxy-middleware-0.19.1.tgz"
   ...
   dependencies:
     http-proxy "^1.17.0"
@@ -5145,7 +5236,7 @@ http-proxy-middleware@0.19.1:
...
+  resolved "https://registry.npmjs.org/http-proxy/-/http-proxy-1.18.0.tgz"
   ...
   dependencies:
     eventemitter3 "^4.0.0"
@@ -5154,7 +5245,7 @@ http-proxy@^1.17.0:
...
+  resolved "https://registry.npmjs.org/http-signature/-/http-signature-1.2.0.tgz"
   ...
   dependencies:
     assert-plus "^1.0.0"
@@ -5163,87 +5254,97 @@ http-signature@~1.2.0:
...
-immer@1.8.2:
-  version "1.8.2"
-  resolved "https://registry.yarnpkg.com/immer/-/immer-1.8.2.tgz#111e9c9e6832c833868f3f6481758a421f63f403"
...
+immer@1.8.2:
+  version "1.8.2"
+  resolved "https://registry.npmjs.org/immer/-/immer-1.8.2.tgz"
+  integrity sha512-uP9EI4hmRTlZ+d4YYmd7JMlxf8I3KPHnvWfLNjpOv/cQNRV/dKHL8L3cZ7z6OCuRQnK59Uu5LJr/KxViqAiEQQ==
+
...
+  resolved "https://registry.npmjs.org/import-fresh/-/import-fresh-2.0.0.tgz"
   ...
   dependencies:
     caller-path "^2.0.0"
@@ -5251,7 +5352,7 @@ import-fresh@^2.0.0:
...
+  resolved "https://registry.npmjs.org/import-fresh/-/import-fresh-3.2.1.tgz"
   ...
   dependencies:
     parent-module "^1.0.0"
@@ -5259,19 +5360,19 @@ import-fresh@^3.0.0, import-fresh@^3.1.0:
...
+  resolved "https://registry.npmjs.org/import-local/-/import-local-2.0.0.tgz"
   ...
   dependencies:
     pkg-dir "^3.0.0"
@@ -5279,72 +5380,77 @@ import-local@2.0.0, import-local@^2.0.0:
...
+inherits@^2.0.1, inherits@^2.0.3, inherits@~2.0.0, inherits@~2.0.1, inherits@~2.0.3, inherits@2, inherits@2.0.4:
   version "2.0.4"
...
+  resolved "https://registry.npmjs.org/internal-ip/-/internal-ip-4.3.0.tgz"
   ...
   dependencies:
     default-gateway "^4.2.0"
@@ -5371,112 +5477,112 @@ internal-ip@^4.3.0:
...
+  resolved "https://registry.npmjs.org/is-alphanumerical/-/is-alphanumerical-1.0.3.tgz"
   ...
   dependencies:
     is-alphabetical "^1.0.0"
@@ -5484,51 +5590,51 @@ is-alphanumerical@^1.0.0:
...
@@ -5540,31 +5646,31 @@ is-color-stop@^1.0.0:
...
+  resolved "https://registry.npmjs.org/is-descriptor/-/is-descriptor-0.1.6.tgz"
   ...
   dependencies:
     is-accessor-descriptor "^0.1.6"
@@ -5573,7 +5679,7 @@ is-descriptor@^0.1.0:
...
+  resolved "https://registry.npmjs.org/is-descriptor/-/is-descriptor-1.0.2.tgz"
   ...
   dependencies:
     is-accessor-descriptor "^1.0.0"
@@ -5582,242 +5688,242 @@ is-descriptor@^1.0.0, is-descriptor@^1.0.2:
...
+  resolved "https://registry.npmjs.org/isomorphic-fetch/-/isomorphic-fetch-2.2.1.tgz"
   ...
   dependencies:
     node-fetch "^1.0.1"
@@ -5825,17 +5931,17 @@ isomorphic-fetch@^2.1.1:
...
@@ -5848,7 +5954,7 @@ istanbul-lib-instrument@^3.0.1, istanbul-lib-instrument@^3.3.0:
...
@@ -5857,7 +5963,7 @@ istanbul-lib-report@^2.0.4:
...
+  resolved "https://registry.npmjs.org/istanbul-lib-source-maps/-/istanbul-lib-source-maps-3.0.6.tgz"
   ...
   dependencies:
     debug "^4.1.1"
@@ -5868,14 +5974,14 @@ istanbul-lib-source-maps@^3.0.1:
...
+  resolved "https://registry.npmjs.org/jest-changed-files/-/jest-changed-files-24.9.0.tgz"
   ...
   dependencies:
     "@jest/types" "^24.9.0"
@@ -5884,7 +5990,7 @@ jest-changed-files@^24.9.0:
...
+  resolved "https://registry.npmjs.org/jest-config/-/jest-config-24.9.0.tgz"
   ...
   dependencies:
     "@babel/core" "^7.1.0"
@@ -5926,7 +6032,7 @@ jest-config@^24.9.0:
...
+  resolved "https://registry.npmjs.org/jest-diff/-/jest-diff-24.9.0.tgz"
   ...
   dependencies:
     chalk "^2.0.1"
@@ -5936,14 +6042,14 @@ jest-diff@^24.0.0, jest-diff@^24.9.0:
...
+  resolved "https://registry.npmjs.org/jest-each/-/jest-each-24.9.0.tgz"
   ...
   dependencies:
     "@jest/types" "^24.9.0"
@@ -5968,7 +6074,7 @@ jest-each@^24.9.0:
...
+  resolved "https://registry.npmjs.org/jest-environment-jsdom/-/jest-environment-jsdom-24.9.0.tgz"
   ...
   dependencies:
     "@jest/environment" "^24.9.0"
@@ -5980,7 +6086,7 @@ jest-environment-jsdom@^24.9.0:
...
+  resolved "https://registry.npmjs.org/jest-environment-node/-/jest-environment-node-24.9.0.tgz"
   ...
   dependencies:
     "@jest/environment" "^24.9.0"
@@ -5991,7 +6097,7 @@ jest-environment-node@^24.9.0:
...
+  resolved "https://registry.npmjs.org/jest-fetch-mock/-/jest-fetch-mock-2.1.2.tgz"
   ...
   dependencies:
     cross-fetch "^2.2.2"
@@ -5999,12 +6105,12 @@ jest-fetch-mock@^2.0.1:
...
+  resolved "https://registry.npmjs.org/jest-haste-map/-/jest-haste-map-24.9.0.tgz"
   ...
   dependencies:
     "@jest/types" "^24.9.0"
@@ -6023,7 +6129,7 @@ jest-haste-map@^24.9.0:
...
@@ -6045,7 +6151,7 @@ jest-jasmine2@^24.9.0:
...
+  resolved "https://registry.npmjs.org/jest-leak-detector/-/jest-leak-detector-24.9.0.tgz"
   ...
   dependencies:
     jest-get-type "^24.9.0"
@@ -6053,7 +6159,7 @@ jest-leak-detector@^24.9.0:
...
+  resolved "https://registry.npmjs.org/jest-matcher-utils/-/jest-matcher-utils-24.9.0.tgz"
   ...
   dependencies:
     chalk "^2.0.1"
@@ -6063,7 +6169,7 @@ jest-matcher-utils@^24.0.0, jest-matcher-utils@^24.9.0:
...
+  resolved "https://registry.npmjs.org/jest-message-util/-/jest-message-util-24.9.0.tgz"
   ...
   dependencies:
     "@babel/code-frame" "^7.0.0"
@@ -6077,33 +6183,33 @@ jest-message-util@^24.9.0:
...
+  resolved "https://registry.npmjs.org/jest-resolve/-/jest-resolve-24.9.0.tgz"
   ...
   dependencies:
     "@jest/types" "^24.9.0"
@@ -6114,7 +6220,7 @@ jest-resolve@^24.9.0:
...
+  resolved "https://registry.npmjs.org/jest-runner/-/jest-runner-24.9.0.tgz"
   ...
   dependencies:
     "@jest/console" "^24.7.1"
@@ -6139,7 +6245,7 @@ jest-runner@^24.9.0:
...
+  resolved "https://registry.npmjs.org/jest-runtime/-/jest-runtime-24.9.0.tgz"
   ...
   dependencies:
     "@jest/console" "^24.7.1"
@@ -6168,12 +6274,12 @@ jest-runtime@^24.9.0:
...
+  resolved "https://registry.npmjs.org/jest-snapshot/-/jest-snapshot-24.9.0.tgz"
   ...
   dependencies:
     "@babel/types" "^7.0.0"
@@ -6192,7 +6298,7 @@ jest-snapshot@^24.9.0:
...
+  resolved "https://registry.npmjs.org/jest-util/-/jest-util-24.9.0.tgz"
   ...
   dependencies:
     "@jest/console" "^24.9.0"
@@ -6210,7 +6316,7 @@ jest-util@^24.9.0:
...
+  resolved "https://registry.npmjs.org/jest-validate/-/jest-validate-24.9.0.tgz"
   ...
   dependencies:
     "@jest/types" "^24.9.0"
@@ -6222,7 +6328,7 @@ jest-validate@^24.9.0:
...
+  resolved "https://registry.npmjs.org/jest-watcher/-/jest-watcher-24.9.0.tgz"
   ...
   dependencies:
     "@jest/test-result" "^24.9.0"
@@ -6235,7 +6341,7 @@ jest-watcher@^24.9.0:
...
+  resolved "https://registry.npmjs.org/jest-worker/-/jest-worker-24.9.0.tgz"
   ...
   dependencies:
     merge-stream "^2.0.0"
@@ -6243,7 +6349,7 @@ jest-worker@^24.6.0, jest-worker@^24.9.0:
...
+  resolved "https://registry.npmjs.org/js-tokens/-/js-tokens-4.0.0.tgz"
 ...
 js-yaml@^3.13.0, js-yaml@^3.13.1, js-yaml@^3.5.2, js-yaml@^3.7.0:
   version "3.13.1"
...
+  resolved "https://registry.npmjs.org/js-yaml/-/js-yaml-3.13.1.tgz"
   ...
   dependencies:
     argparse "^1.0.7"
@@ -6274,12 +6380,12 @@ js-yaml@^3.13.0, js-yaml@^3.13.1, js-yaml@^3.5.2, js-yaml@^3.7.0:
...
+  resolved "https://registry.npmjs.org/jsx-ast-utils/-/jsx-ast-utils-2.2.3.tgz"
   ...
   dependencies:
     array-includes "^3.0.3"
@@ -6388,51 +6494,65 @@ jsx-ast-utils@^2.2.1:
...
+  resolved "https://registry.npmjs.org/last-call-webpack-plugin/-/last-call-webpack-plugin-3.0.0.tgz"
   ...
   dependencies:
     lodash "^4.17.5"
@@ -6440,36 +6560,36 @@ last-call-webpack-plugin@^3.0.0:
...
+  resolved "https://registry.npmjs.org/levn/-/levn-0.3.0.tgz"
   ...
   dependencies:
     prelude-ls "~1.1.2"
@@ -6477,19 +6597,19 @@ levn@^0.3.0, levn@~0.3.0:
...
+  resolved "https://registry.npmjs.org/load-json-file/-/load-json-file-1.1.0.tgz"
   ...
   dependencies:
     graceful-fs "^4.1.2"
@@ -6500,7 +6620,7 @@ load-json-file@^1.0.0:
...
+  resolved "https://registry.npmjs.org/load-json-file/-/load-json-file-2.0.0.tgz"
   ...
   dependencies:
     graceful-fs "^4.1.2"
@@ -6510,7 +6630,7 @@ load-json-file@^2.0.0:
...
+  resolved "https://registry.npmjs.org/load-json-file/-/load-json-file-4.0.0.tgz"
   ...
   dependencies:
     graceful-fs "^4.1.2"
@@ -6520,17 +6640,17 @@ load-json-file@^4.0.0:
...
+loader-utils@^1.0.1, loader-utils@^1.0.2, loader-utils@^1.1.0, loader-utils@^1.2.3, loader-utils@1.2.3:
   version "1.2.3"
...
+  resolved "https://registry.npmjs.org/loader-utils/-/loader-utils-1.2.3.tgz"
   ...
   dependencies:
     big.js "^5.2.2"
@@ -6539,7 +6659,7 @@ loader-utils@1.2.3, loader-utils@^1.0.1, loader-utils@^1.0.2, loader-utils@^1.1.
...
+  resolved "https://registry.npmjs.org/locate-path/-/locate-path-2.0.0.tgz"
   ...
   dependencies:
     p-locate "^2.0.0"
@@ -6547,7 +6667,7 @@ locate-path@^2.0.0:
...
+  resolved "https://registry.npmjs.org/locate-path/-/locate-path-3.0.0.tgz"
   ...
   dependencies:
     p-locate "^3.0.0"
@@ -6555,49 +6675,49 @@ locate-path@^3.0.0:
...
+  resolved "https://registry.npmjs.org/lodash.template/-/lodash.template-4.5.0.tgz"
   ...
   dependencies:
     lodash._reinterpolate "^3.0.0"
@@ -6605,58 +6725,58 @@ lodash.template@^4.2.4, lodash.template@^4.5.0:
...
+  resolved "https://registry.npmjs.org/loud-rejection/-/loud-rejection-1.6.0.tgz"
   ...
   dependencies:
     currently-unhandled "^0.4.1"
@@ -6664,19 +6784,19 @@ loud-rejection@^1.0.0:
...
+  resolved "https://registry.npmjs.org/lru-cache/-/lru-cache-4.1.5.tgz"
   ...
   dependencies:
     pseudomap "^1.0.2"
@@ -6684,14 +6804,14 @@ lru-cache@^4.0.1:
...
+  resolved "https://registry.npmjs.org/make-dir/-/make-dir-2.1.0.tgz"
   ...
   dependencies:
     pify "^4.0.1"
@@ -6699,55 +6819,55 @@ make-dir@^2.0.0, make-dir@^2.1.0:
...
-mapbox-gl@~0.54.0:
+"mapbox-gl@>=0.32.1 <2.0.0", mapbox-gl@~0.54.0:
   version "0.54.1"
...
+  resolved "https://registry.npmjs.org/mapbox-gl/-/mapbox-gl-0.54.1.tgz"
   ...
   dependencies:
     "@mapbox/geojson-rewind" "^0.4.0"
@@ -6777,34 +6897,34 @@ mapbox-gl@~0.54.0:
 
 markdown-draft-js@^1.3.1:
   version "1.7.1"
-  resolved "https://registry.yarnpkg.com/markdown-draft-js/-/markdown-draft-js-1.7.1.tgz#893ddf0f9739cf44d69c66d5691add42e7af0cb7"
+  resolved "https://registry.npmjs.org/markdown-draft-js/-/markdown-draft-js-1.7.1.tgz"
   integrity sha512-rr5EtEy7eGm8Wok090Pi/Yj0r5TSwMmBqNKcnvE0/h1LrLJcQARZlf8/rYpyRDS4Fnq7ao1rtisZ6J4RjCF90Q==
   dependencies:
     remarkable "1.7.1"
...
+  resolved "https://registry.npmjs.org/memory-fs/-/memory-fs-0.4.1.tgz"
   ...
   dependencies:
     errno "^0.1.3"
@@ -6859,7 +6979,7 @@ memory-fs@^0.4.0, memory-fs@^0.4.1:
...
+  resolved "https://registry.npmjs.org/micromatch/-/micromatch-3.1.10.tgz"
   ...
   dependencies:
     arr-diff "^4.0.0"
@@ -6944,47 +7064,47 @@ micromatch@^3.0.4, micromatch@^3.1.10, micromatch@^3.1.4:
...
+  resolved "https://registry.npmjs.org/mini-css-extract-plugin/-/mini-css-extract-plugin-0.8.0.tgz"
   ...
   dependencies:
     loader-utils "^1.1.0"
@@ -6994,100 +7114,96 @@ mini-css-extract-plugin@^0.8.0:
...
+  resolved "https://registry.npmjs.org/mississippi/-/mississippi-3.0.0.tgz"
   ...
   dependencies:
     concat-stream "^1.5.0"
@@ -7103,7 +7219,7 @@ mississippi@^3.0.0:
...
+  resolved "https://registry.npmjs.org/mixin-deep/-/mixin-deep-1.3.2.tgz"
   ...
   dependencies:
     for-in "^1.0.2"
@@ -7111,22 +7227,22 @@ mixin-deep@^1.2.0:
...
+  resolved "https://registry.npmjs.org/move-concurrently/-/move-concurrently-1.0.1.tgz"
   ...
   dependencies:
     aproba "^1.1.1"
@@ -7136,29 +7252,29 @@ move-concurrently@^1.0.1:
     rimraf "^2.5.4"
     run-queue "^1.0.3"
...
+  resolved "https://registry.npmjs.org/multicast-dns/-/multicast-dns-6.2.3.tgz"
   ...
   dependencies:
     dns-packet "^1.3.1"
@@ -7166,22 +7282,22 @@ multicast-dns@^6.0.1:
...
+  resolved "https://registry.npmjs.org/nanomatch/-/nanomatch-1.2.13.tgz"
   ...
   dependencies:
     arr-diff "^4.0.0"
@@ -7198,61 +7314,59 @@ nanomatch@^1.2.9:
...
+  resolved "https://registry.npmjs.org/node-gyp/-/node-gyp-3.8.0.tgz"
   ...
   dependencies:
     fstream "^1.0.0"
@@ -7270,12 +7384,12 @@ node-gyp@^3.8.0:
...
+  resolved "https://registry.npmjs.org/node-libs-browser/-/node-libs-browser-2.2.1.tgz"
   ...
   dependencies:
     assert "^1.1.1"
@@ -7304,12 +7418,12 @@ node-libs-browser@^2.2.1:
...
+  resolved "https://registry.npmjs.org/node-notifier/-/node-notifier-5.4.3.tgz"
   ...
   dependencies:
     growly "^1.3.0"
@@ -7320,7 +7434,7 @@ node-notifier@^5.4.2:
...
+  resolved "https://registry.npmjs.org/node-pre-gyp/-/node-pre-gyp-0.12.0.tgz"
   ...
   dependencies:
     detect-libc "^1.0.2"
@@ -7336,14 +7450,14 @@ node-pre-gyp@^0.12.0:
...
+  resolved "https://registry.npmjs.org/node-sass/-/node-sass-4.12.0.tgz"
   ...
   dependencies:
     async-foreach "^0.1.3"
@@ -7366,7 +7480,7 @@ node-sass@4.12.0:
...
+  resolved "https://registry.npmjs.org/node-sass/-/node-sass-4.13.0.tgz"
   ...
   dependencies:
     async-foreach "^0.1.3"
@@ -7387,24 +7501,24 @@ node-sass@^4.13.0:
     stdout-stream "^1.4.0"
...
+  resolved "https://registry.npmjs.org/normalize-package-data/-/normalize-package-data-2.5.0.tgz"
   ...
   dependencies:
     hosted-git-info "^2.1.4"
@@ -7414,29 +7528,34 @@ normalize-package-data@^2.3.2, normalize-package-data@^2.3.4:
...
+  resolved "https://registry.npmjs.org/normalize-url/-/normalize-url-1.9.1.tgz"
   ...
   dependencies:
     object-assign "^4.0.1"
@@ -7444,39 +7563,42 @@ normalize-url@1.9.1:
     query-string "^4.1.0"
     sort-keys "^1.0.0"
...
+  resolved "https://registry.npmjs.org/npmlog/-/npmlog-4.1.2.tgz"
   ...
   dependencies:
     are-we-there-yet "~1.1.2"
@@ -7486,39 +7608,39 @@ npm-run-path@^2.0.0:
...
+  resolved "https://registry.npmjs.org/object-copy/-/object-copy-0.1.0.tgz"
   ...
   dependencies:
     copy-descriptor "^0.1.0"
@@ -7527,29 +7649,29 @@ object-copy@^0.1.0:
...
+  resolved "https://registry.npmjs.org/object.assign/-/object.assign-4.1.0.tgz"
   ...
   dependencies:
     define-properties "^1.1.2"
@@ -7559,7 +7681,7 @@ object.assign@^4.1.0:
...
@@ -7569,7 +7691,7 @@ object.entries@^1.1.0:
...
+  resolved "https://registry.npmjs.org/object.fromentries/-/object.fromentries-2.0.1.tgz"
   ...
   dependencies:
     define-properties "^1.1.3"
@@ -7579,7 +7701,7 @@ object.fromentries@^2.0.0:
...
+  resolved "https://registry.npmjs.org/object.getownpropertydescriptors/-/object.getownpropertydescriptors-2.0.3.tgz"
   ...
   dependencies:
     define-properties "^1.1.2"
@@ -7587,14 +7709,14 @@ object.getownpropertydescriptors@^2.0.3:
...
+  resolved "https://registry.npmjs.org/object.values/-/object.values-1.1.0.tgz"
   ...
   dependencies:
     define-properties "^1.1.3"
@@ -7604,55 +7726,55 @@ object.values@^1.1.0:
...
+  resolved "https://registry.npmjs.org/optimize-css-assets-webpack-plugin/-/optimize-css-assets-webpack-plugin-5.0.3.tgz"
   ...
   dependencies:
     cssnano "^4.1.10"
@@ -7668,7 +7790,7 @@ optimize-css-assets-webpack-plugin@^5.0.3:
...
+  resolved "https://registry.npmjs.org/optionator/-/optionator-0.8.3.tgz"
   ...
   dependencies:
     deep-is "~0.1.3"
@@ -7680,31 +7802,31 @@ optionator@^0.8.1, optionator@^0.8.2:
...
+  resolved "https://registry.npmjs.org/os-locale/-/os-locale-3.1.0.tgz"
   ...
   dependencies:
     execa "^1.0.0"
@@ -7713,12 +7835,20 @@ os-locale@^3.0.0, os-locale@^3.1.0:
...
+  resolved "https://registry.npmjs.org/osenv/-/osenv-0.1.5.tgz"
   ...
   dependencies:
     os-homedir "^1.0.0"
@@ -7726,103 +7856,103 @@ osenv@0, osenv@^0.1.4:
...
+  resolved "https://registry.npmjs.org/parallel-transform/-/parallel-transform-1.2.0.tgz"
   ...
   dependencies:
     cyclist "^1.0.1"
@@ -7831,21 +7961,21 @@ parallel-transform@^1.1.0:
...
+  resolved "https://registry.npmjs.org/parse-asn1/-/parse-asn1-5.1.5.tgz"
   ...
   dependencies:
     asn1.js "^4.0.0"
@@ -7857,7 +7987,7 @@ parse-asn1@^5.0.0:
...
+  resolved "https://registry.npmjs.org/parse-entities/-/parse-entities-1.2.2.tgz"
   ...
   dependencies:
     character-entities "^1.0.0"
@@ -7869,14 +7999,14 @@ parse-entities@^1.0.2, parse-entities@^1.1.0:
...
+  resolved "https://registry.npmjs.org/parse-json/-/parse-json-4.0.0.tgz"
   ...
   dependencies:
     error-ex "^1.3.1"
@@ -7884,7 +8014,7 @@ parse-json@^4.0.0:
...
+  resolved "https://registry.npmjs.org/parse-json/-/parse-json-5.0.0.tgz"
   ...
   dependencies:
     "@babel/code-frame" "^7.0.0"
@@ -7894,22 +8024,22 @@ parse-json@^5.0.0:
...
+  resolved "https://registry.npmjs.org/pascal-case/-/pascal-case-2.0.1.tgz"
   ...
   dependencies:
     camel-case "^3.0.0"
@@ -7917,88 +8047,88 @@ pascal-case@^2.0.0:
...
+  resolved "https://registry.npmjs.org/path-type/-/path-type-1.1.0.tgz"
   ...
   dependencies:
     graceful-fs "^4.1.2"
@@ -8007,26 +8137,26 @@ path-type@^1.0.0:
...
+  resolved "https://registry.npmjs.org/pleeease-filters/-/pleeease-filters-4.0.0.tgz"
   ...
   dependencies:
     onecolor "^3.0.4"
@@ -8122,31 +8257,31 @@ pleeease-filters@^4.0.0:
...
+  resolved "https://registry.npmjs.org/polished/-/polished-2.3.3.tgz"
 ...
 popper.js@^1.12.9, popper.js@^1.14.1, popper.js@^1.14.4:
   version "1.16.0"
...
+  resolved "https://registry.npmjs.org/portfinder/-/portfinder-1.0.25.tgz"
   ...
   dependencies:
     async "^2.6.2"
@@ -8155,12 +8290,12 @@ portfinder@^1.0.25:
...
@@ -8169,7 +8304,7 @@ postcss-apply@^0.8.0:
...
+  resolved "https://registry.npmjs.org/postcss-attribute-case-insensitive/-/postcss-attribute-case-insensitive-2.0.0.tgz"
   ...
   dependencies:
     postcss "^6.0.0"
@@ -8177,7 +8312,7 @@ postcss-attribute-case-insensitive@^2.0.0:
...
+  resolved "https://registry.npmjs.org/postcss-attribute-case-insensitive/-/postcss-attribute-case-insensitive-4.0.1.tgz"
   ...
   dependencies:
     postcss "^7.0.2"
@@ -8185,7 +8320,7 @@ postcss-attribute-case-insensitive@^4.0.1:
...
+  resolved "https://registry.npmjs.org/postcss-calc/-/postcss-calc-6.0.2.tgz"
   ...
   dependencies:
     css-unit-converter "^1.1.1"
@@ -8195,7 +8330,7 @@ postcss-calc@^6.0.0:
...
+  resolved "https://registry.npmjs.org/postcss-calc/-/postcss-calc-7.0.1.tgz"
   ...
   dependencies:
     css-unit-converter "^1.1.1"
@@ -8205,7 +8340,7 @@ postcss-calc@^7.0.1:
...
+  resolved "https://registry.npmjs.org/postcss-color-function/-/postcss-color-function-4.1.0.tgz"
   ...
   dependencies:
     css-color-function "~1.3.3"
@@ -8215,7 +8350,7 @@ postcss-color-function@^4.0.0:
...
+  resolved "https://registry.npmjs.org/postcss-color-functional-notation/-/postcss-color-functional-notation-2.0.1.tgz"
   ...
   dependencies:
     postcss "^7.0.2"
@@ -8223,7 +8358,7 @@ postcss-color-functional-notation@^2.0.1:
...
+  resolved "https://registry.npmjs.org/postcss-color-gray/-/postcss-color-gray-4.1.0.tgz"
   ...
   dependencies:
     color "^2.0.1"
@@ -8233,7 +8368,7 @@ postcss-color-gray@^4.0.0:
...
+  resolved "https://registry.npmjs.org/postcss-color-gray/-/postcss-color-gray-5.0.0.tgz"
   ...
   dependencies:
     "@csstools/convert-colors" "^1.4.0"
@@ -8242,7 +8377,7 @@ postcss-color-gray@^5.0.0:
...
+  resolved "https://registry.npmjs.org/postcss-color-hex-alpha/-/postcss-color-hex-alpha-3.0.0.tgz"
   ...
   dependencies:
     color "^1.0.3"
@@ -8251,7 +8386,7 @@ postcss-color-hex-alpha@^3.0.0:
...
+  resolved "https://registry.npmjs.org/postcss-color-hex-alpha/-/postcss-color-hex-alpha-5.0.3.tgz"
   ...
   dependencies:
     postcss "^7.0.14"
@@ -8259,7 +8394,7 @@ postcss-color-hex-alpha@^5.0.3:
...
+  resolved "https://registry.npmjs.org/postcss-color-hsl/-/postcss-color-hsl-2.0.0.tgz"
   ...
   dependencies:
     postcss "^6.0.1"
@@ -8268,7 +8403,7 @@ postcss-color-hsl@^2.0.0:
...
+  resolved "https://registry.npmjs.org/postcss-color-hwb/-/postcss-color-hwb-3.0.0.tgz"
   ...
   dependencies:
     color "^1.0.3"
@@ -8278,7 +8413,7 @@ postcss-color-hwb@^3.0.0:
...
+  resolved "https://registry.npmjs.org/postcss-color-mod-function/-/postcss-color-mod-function-3.0.3.tgz"
   ...
   dependencies:
     "@csstools/convert-colors" "^1.4.0"
@@ -8287,7 +8422,7 @@ postcss-color-mod-function@^3.0.3:
...
+  resolved "https://registry.npmjs.org/postcss-color-rebeccapurple/-/postcss-color-rebeccapurple-3.1.0.tgz"
   ...
   dependencies:
     postcss "^6.0.22"
@@ -8295,7 +8430,7 @@ postcss-color-rebeccapurple@^3.0.0:
...
+  resolved "https://registry.npmjs.org/postcss-color-rebeccapurple/-/postcss-color-rebeccapurple-4.0.1.tgz"
   ...
   dependencies:
     postcss "^7.0.2"
@@ -8303,7 +8438,7 @@ postcss-color-rebeccapurple@^4.0.1:
...
+  resolved "https://registry.npmjs.org/postcss-color-rgb/-/postcss-color-rgb-2.0.0.tgz"
   ...
   dependencies:
     postcss "^6.0.1"
@@ -8311,7 +8446,7 @@ postcss-color-rgb@^2.0.0:
...
+  resolved "https://registry.npmjs.org/postcss-color-rgba-fallback/-/postcss-color-rgba-fallback-3.0.0.tgz"
   ...
   dependencies:
     postcss "^6.0.6"
@@ -8320,7 +8455,7 @@ postcss-color-rgba-fallback@^3.0.0:
...
+  resolved "https://registry.npmjs.org/postcss-colormin/-/postcss-colormin-4.0.3.tgz"
   ...
   dependencies:
     browserslist "^4.0.0"
@@ -8331,7 +8466,7 @@ postcss-colormin@^4.0.3:
...
+  resolved "https://registry.npmjs.org/postcss-convert-values/-/postcss-convert-values-4.0.1.tgz"
   ...
   dependencies:
     postcss "^7.0.0"
@@ -8339,7 +8474,7 @@ postcss-convert-values@^4.0.1:
...
+  resolved "https://registry.npmjs.org/postcss-cssnext/-/postcss-cssnext-3.1.0.tgz"
   ...
   dependencies:
     autoprefixer "^7.1.1"
@@ -8376,21 +8511,21 @@ postcss-cssnext@^3.1.0:
...
+  resolved "https://registry.npmjs.org/postcss-custom-properties/-/postcss-custom-properties-6.3.1.tgz"
   ...
   dependencies:
     balanced-match "^1.0.0"
@@ -8398,7 +8533,7 @@ postcss-custom-properties@^6.1.0:
...
+  resolved "https://registry.npmjs.org/postcss-custom-properties/-/postcss-custom-properties-8.0.11.tgz"
   ...
   dependencies:
     postcss "^7.0.17"
@@ -8406,7 +8541,7 @@ postcss-custom-properties@^8.0.11:
...
+  resolved "https://registry.npmjs.org/postcss-custom-selectors/-/postcss-custom-selectors-4.0.1.tgz"
   ...
   dependencies:
     postcss "^6.0.1"
@@ -8414,7 +8549,7 @@ postcss-custom-selectors@^4.0.1:
...
+  resolved "https://registry.npmjs.org/postcss-custom-selectors/-/postcss-custom-selectors-5.1.2.tgz"
   ...
   dependencies:
     postcss "^7.0.2"
@@ -8422,7 +8557,7 @@ postcss-custom-selectors@^5.1.2:
...
+  resolved "https://registry.npmjs.org/postcss-dir-pseudo-class/-/postcss-dir-pseudo-class-5.0.0.tgz"
   ...
   dependencies:
     postcss "^7.0.2"
@@ -8430,35 +8565,35 @@ postcss-dir-pseudo-class@^5.0.0:
...
+  resolved "https://registry.npmjs.org/postcss-double-position-gradients/-/postcss-double-position-gradients-1.0.0.tgz"
   ...
   dependencies:
     postcss "^7.0.5"
@@ -8466,7 +8601,7 @@ postcss-double-position-gradients@^1.0.0:
...
+  resolved "https://registry.npmjs.org/postcss-env-function/-/postcss-env-function-2.0.2.tgz"
   ...
   dependencies:
     postcss "^7.0.2"
@@ -8474,63 +8609,63 @@ postcss-env-function@^2.0.2:
...
+  resolved "https://registry.npmjs.org/postcss-image-set-function/-/postcss-image-set-function-3.0.1.tgz"
   ...
   dependencies:
     postcss "^7.0.2"
@@ -8538,7 +8673,7 @@ postcss-image-set-function@^3.0.1:
...
+  resolved "https://registry.npmjs.org/postcss-image-set-polyfill/-/postcss-image-set-polyfill-0.3.5.tgz"
   ...
   dependencies:
     postcss "^6.0.1"
@@ -8546,7 +8681,7 @@ postcss-image-set-polyfill@^0.3.5:
...
+  resolved "https://registry.npmjs.org/postcss-import/-/postcss-import-12.0.1.tgz"
   ...
   dependencies:
     postcss "^7.0.1"
@@ -8556,7 +8691,7 @@ postcss-import@^12.0.1:
...
@@ -8564,7 +8699,7 @@ postcss-initial@^2.0.0:
...
@@ -8572,14 +8707,14 @@ postcss-initial@^3.0.0:
...
+  resolved "https://registry.npmjs.org/postcss-lab-function/-/postcss-lab-function-2.0.1.tgz"
   ...
   dependencies:
     "@csstools/convert-colors" "^1.4.0"
@@ -8588,14 +8723,14 @@ postcss-lab-function@^2.0.1:
...
+  resolved "https://registry.npmjs.org/postcss-load-config/-/postcss-load-config-2.1.0.tgz"
   ...
   dependencies:
     cosmiconfig "^5.0.0"
@@ -8603,7 +8738,7 @@ postcss-load-config@^2.0.0:
...
+  resolved "https://registry.npmjs.org/postcss-loader/-/postcss-loader-3.0.0.tgz"
   ...
   dependencies:
     loader-utils "^1.1.0"
@@ -8613,14 +8748,14 @@ postcss-loader@^3.0.0:
...
+  resolved "https://registry.npmjs.org/postcss-markdown/-/postcss-markdown-0.36.0.tgz"
   ...
   dependencies:
     remark "^10.0.1"
@@ -8628,26 +8763,26 @@ postcss-markdown@^0.36.0:
...
+  resolved "https://registry.npmjs.org/postcss-merge-longhand/-/postcss-merge-longhand-4.0.11.tgz"
   ...
   dependencies:
     css-color-names "0.0.4"
@@ -8657,7 +8792,7 @@ postcss-merge-longhand@^4.0.11:
...
+  resolved "https://registry.npmjs.org/postcss-merge-rules/-/postcss-merge-rules-4.0.3.tgz"
   ...
   dependencies:
     browserslist "^4.0.0"
@@ -8669,12 +8804,12 @@ postcss-merge-rules@^4.0.3:
...
+  resolved "https://registry.npmjs.org/postcss-minify-font-values/-/postcss-minify-font-values-4.0.2.tgz"
   ...
   dependencies:
     postcss "^7.0.0"
@@ -8682,7 +8817,7 @@ postcss-minify-font-values@^4.0.2:
...
+  resolved "https://registry.npmjs.org/postcss-minify-gradients/-/postcss-minify-gradients-4.0.2.tgz"
   ...
   dependencies:
     cssnano-util-get-arguments "^4.0.0"
@@ -8692,7 +8827,7 @@ postcss-minify-gradients@^4.0.2:
...
+  resolved "https://registry.npmjs.org/postcss-minify-params/-/postcss-minify-params-4.0.2.tgz"
   ...
   dependencies:
     alphanum-sort "^1.0.0"
@@ -8704,7 +8839,7 @@ postcss-minify-params@^4.0.2:
...
+  resolved "https://registry.npmjs.org/postcss-minify-selectors/-/postcss-minify-selectors-4.0.2.tgz"
   ...
   dependencies:
     alphanum-sort "^1.0.0"
@@ -8714,14 +8849,14 @@ postcss-minify-selectors@^4.0.2:
...
+  resolved "https://registry.npmjs.org/postcss-modules-local-by-default/-/postcss-modules-local-by-default-2.0.6.tgz"
   ...
   dependencies:
     postcss "^7.0.6"
@@ -8730,7 +8865,7 @@ postcss-modules-local-by-default@^2.0.6:
...
+  resolved "https://registry.npmjs.org/postcss-modules-local-by-default/-/postcss-modules-local-by-default-3.0.2.tgz"
   ...
   dependencies:
     icss-utils "^4.1.1"
@@ -8740,7 +8875,7 @@ postcss-modules-local-by-default@^3.0.2:
...
+  resolved "https://registry.npmjs.org/postcss-modules-scope/-/postcss-modules-scope-2.1.0.tgz"
   ...
   dependencies:
     postcss "^7.0.6"
@@ -8748,7 +8883,7 @@ postcss-modules-scope@^2.1.0:
...
+  resolved "https://registry.npmjs.org/postcss-modules-values/-/postcss-modules-values-2.0.0.tgz"
   ...
   dependencies:
     icss-replace-symbols "^1.1.0"
@@ -8756,7 +8891,7 @@ postcss-modules-values@^2.0.0:
...
+  resolved "https://registry.npmjs.org/postcss-modules-values/-/postcss-modules-values-3.0.0.tgz"
   ...
   dependencies:
     icss-utils "^4.0.0"
@@ -8764,28 +8899,28 @@ postcss-modules-values@^3.0.0:
...
+  resolved "https://registry.npmjs.org/postcss-normalize-display-values/-/postcss-normalize-display-values-4.0.2.tgz"
   ...
   dependencies:
     cssnano-util-get-match "^4.0.0"
@@ -8794,7 +8929,7 @@ postcss-normalize-display-values@^4.0.2:
...
+  resolved "https://registry.npmjs.org/postcss-normalize-positions/-/postcss-normalize-positions-4.0.2.tgz"
   ...
   dependencies:
     cssnano-util-get-arguments "^4.0.0"
@@ -8804,7 +8939,7 @@ postcss-normalize-positions@^4.0.2:
...
+  resolved "https://registry.npmjs.org/postcss-normalize-repeat-style/-/postcss-normalize-repeat-style-4.0.2.tgz"
   ...
   dependencies:
     cssnano-util-get-arguments "^4.0.0"
@@ -8814,7 +8949,7 @@ postcss-normalize-repeat-style@^4.0.2:
...
+  resolved "https://registry.npmjs.org/postcss-normalize-string/-/postcss-normalize-string-4.0.2.tgz"
   ...
   dependencies:
     has "^1.0.0"
@@ -8823,7 +8958,7 @@ postcss-normalize-string@^4.0.2:
...
+  resolved "https://registry.npmjs.org/postcss-normalize-timing-functions/-/postcss-normalize-timing-functions-4.0.2.tgz"
   ...
   dependencies:
     cssnano-util-get-match "^4.0.0"
@@ -8832,7 +8967,7 @@ postcss-normalize-timing-functions@^4.0.2:
...
+  resolved "https://registry.npmjs.org/postcss-normalize-unicode/-/postcss-normalize-unicode-4.0.1.tgz"
   ...
   dependencies:
     browserslist "^4.0.0"
@@ -8841,7 +8976,7 @@ postcss-normalize-unicode@^4.0.1:
...
+  resolved "https://registry.npmjs.org/postcss-normalize-url/-/postcss-normalize-url-4.0.1.tgz"
   ...
   dependencies:
     is-absolute-url "^2.0.0"
@@ -8851,7 +8986,7 @@ postcss-normalize-url@^4.0.1:
...
+  resolved "https://registry.npmjs.org/postcss-normalize-whitespace/-/postcss-normalize-whitespace-4.0.2.tgz"
   ...
   dependencies:
     postcss "^7.0.0"
@@ -8859,7 +8994,7 @@ postcss-normalize-whitespace@^4.0.2:
...
+  resolved "https://registry.npmjs.org/postcss-ordered-values/-/postcss-ordered-values-4.1.2.tgz"
   ...
   dependencies:
     cssnano-util-get-arguments "^4.0.0"
@@ -8868,21 +9003,21 @@ postcss-ordered-values@^4.1.2:
...
+  resolved "https://registry.npmjs.org/postcss-place/-/postcss-place-4.0.1.tgz"
   ...
   dependencies:
     postcss "^7.0.2"
@@ -8890,7 +9025,7 @@ postcss-place@^4.0.1:
...
+  resolved "https://registry.npmjs.org/postcss-preset-env/-/postcss-preset-env-6.7.0.tgz"
   ...
   dependencies:
     autoprefixer "^9.6.1"
@@ -8933,7 +9068,7 @@ postcss-preset-env@^6.7.0:
...
+  resolved "https://registry.npmjs.org/postcss-pseudo-class-any-link/-/postcss-pseudo-class-any-link-4.0.0.tgz"
   ...
   dependencies:
     postcss "^6.0.1"
@@ -8941,7 +9076,7 @@ postcss-pseudo-class-any-link@^4.0.0:
...
+  resolved "https://registry.npmjs.org/postcss-pseudo-class-any-link/-/postcss-pseudo-class-any-link-6.0.0.tgz"
   ...
   dependencies:
     postcss "^7.0.2"
@@ -8949,14 +9084,14 @@ postcss-pseudo-class-any-link@^6.0.0:
...
+  resolved "https://registry.npmjs.org/postcss-reduce-initial/-/postcss-reduce-initial-4.0.3.tgz"
   ...
   dependencies:
     browserslist "^4.0.0"
@@ -8966,7 +9101,7 @@ postcss-reduce-initial@^4.0.3:
...
+  resolved "https://registry.npmjs.org/postcss-reduce-transforms/-/postcss-reduce-transforms-4.0.2.tgz"
   ...
   dependencies:
     cssnano-util-get-match "^4.0.0"
@@ -8976,21 +9111,21 @@ postcss-reduce-transforms@^4.0.2:
...
+  resolved "https://registry.npmjs.org/postcss-reporter/-/postcss-reporter-6.0.1.tgz"
   ...
   dependencies:
     chalk "^2.4.1"
@@ -9000,19 +9135,19 @@ postcss-reporter@^6.0.0:
...
+  resolved "https://registry.npmjs.org/postcss-sass/-/postcss-sass-0.2.0.tgz"
   ...
   dependencies:
     gonzales-pe "^4.0.3"
@@ -9020,7 +9155,7 @@ postcss-sass@^0.2.0:
...
+  resolved "https://registry.npmjs.org/postcss-sass/-/postcss-sass-0.3.5.tgz"
   ...
   dependencies:
     gonzales-pe "^4.2.3"
@@ -9028,21 +9163,21 @@ postcss-sass@^0.3.5:
...
+  resolved "https://registry.npmjs.org/postcss-selector-matches/-/postcss-selector-matches-3.0.1.tgz"
   ...
   dependencies:
     balanced-match "^0.4.2"
@@ -9050,7 +9185,7 @@ postcss-selector-matches@^3.0.0, postcss-selector-matches@^3.0.1:
...
+  resolved "https://registry.npmjs.org/postcss-selector-matches/-/postcss-selector-matches-4.0.0.tgz"
   ...
   dependencies:
     balanced-match "^1.0.0"
@@ -9058,7 +9193,7 @@ postcss-selector-matches@^4.0.0:
...
+  resolved "https://registry.npmjs.org/postcss-selector-not/-/postcss-selector-not-3.0.1.tgz"
   ...
   dependencies:
     balanced-match "^0.4.2"
@@ -9066,24 +9201,42 @@ postcss-selector-not@^3.0.1:
...
+  resolved "https://registry.npmjs.org/postcss-selector-parser/-/postcss-selector-parser-3.1.1.tgz"
   ...
   dependencies:
     dot-prop "^4.1.1"
@@ -9092,16 +9245,25 @@ postcss-selector-parser@^3.0.0, postcss-selector-parser@^3.1.0:
...
+  resolved "https://registry.npmjs.org/postcss-selector-parser/-/postcss-selector-parser-6.0.2.tgz"
   ...
   dependencies:
     cssesc "^3.0.0"
@@ -9110,7 +9272,7 @@ postcss-selector-parser@^6.0.0, postcss-selector-parser@^6.0.2:
...
@@ -9127,7 +9289,7 @@ postcss-smart-import@^0.7.6:
...
+  resolved "https://registry.npmjs.org/postcss-svgo/-/postcss-svgo-4.0.2.tgz"
   ...
   dependencies:
     is-svg "^3.0.0"
@@ -9135,14 +9297,14 @@ postcss-svgo@^4.0.2:
     postcss-value-parser "^3.0.0"
     svgo "^1.0.0"
 
-postcss-syntax@^0.36.2:
+postcss-syntax@^0.36.2, postcss-syntax@>=0.36.0:
   version "0.36.2"
...
+  resolved "https://registry.npmjs.org/postcss-unique-selectors/-/postcss-unique-selectors-4.0.1.tgz"
   ...
   dependencies:
     alphanum-sort "^1.0.0"
@@ -9151,17 +9313,22 @@ postcss-unique-selectors@^4.0.1:
...
+  resolved "https://registry.npmjs.org/postcss-values-parser/-/postcss-values-parser-1.5.0.tgz"
   ...
   dependencies:
     flatten "^1.0.2"
@@ -9170,25 +9337,106 @@ postcss-values-parser@^1.5.0:
...
-postcss@^7.0.0, postcss@^7.0.1, postcss@^7.0.13, postcss@^7.0.14, postcss@^7.0.16, postcss@^7.0.17, postcss@^7.0.2, postcss@^7.0.23, postcss@^7.0.5, postcss@^7.0.6, postcss@^7.0.7:
+postcss@^6.0.1:
+  version "6.0.23"
+  resolved "https://registry.npmjs.org/postcss/-/postcss-6.0.23.tgz"
+  integrity sha512-soOk1h6J3VMTZtVeVpv15/Hpdl2cBLX3CAw4TAbkpTJiNPk9YP/zWcD1ND+xEtvyuuvKzbxliTOIyvkSeSJ6ag==
...
+    supports-color "^5.4.0"
+
+postcss@^7.0.0, postcss@^7.0.1, postcss@^7.0.13, postcss@^7.0.14, postcss@^7.0.16, postcss@^7.0.17, postcss@^7.0.2, postcss@^7.0.23, postcss@^7.0.5, postcss@^7.0.6, postcss@^7.0.7, postcss@>=5.0.0:
   version "7.0.23"
...
+  resolved "https://registry.npmjs.org/postcss/-/postcss-7.0.23.tgz"
   ...
   dependencies:
     chalk "^2.4.2"
@@ -9197,27 +9445,27 @@ postcss@^7.0.0, postcss@^7.0.1, postcss@^7.0.13, postcss@^7.0.14, postcss@^7.0.1
...
+  resolved "https://registry.npmjs.org/pretty-format/-/pretty-format-24.9.0.tgz"
   ...
   dependencies:
     "@jest/types" "^24.9.0"
@@ -9227,59 +9475,59 @@ pretty-format@^24.0.0, pretty-format@^24.7.0, pretty-format@^24.9.0:
...
-prop-types@^15.5.4, prop-types@^15.5.6, prop-types@^15.5.7, prop-types@^15.5.8, prop-types@^15.6.0, prop-types@^15.6.1, prop-types@^15.6.2, prop-types@^15.7.2:
+prop-types@^15.0.0, prop-types@^15.5.4, prop-types@^15.5.6, prop-types@^15.5.7, prop-types@^15.5.8, prop-types@^15.6.0, prop-types@^15.6.1, prop-types@^15.6.2, prop-types@^15.7.2, "prop-types@<= 15.x.x":
   version "15.7.2"
...
+  resolved "https://registry.npmjs.org/prop-types/-/prop-types-15.7.2.tgz"
   ...
   dependencies:
     loose-envify "^1.4.0"
@@ -9288,12 +9536,12 @@ prop-types@^15.5.4, prop-types@^15.5.6, prop-types@^15.5.7, prop-types@^15.5.8,
...
+  resolved "https://registry.npmjs.org/proxy-addr/-/proxy-addr-2.0.5.tgz"
   ...
   dependencies:
     forwarded "~0.1.2"
@@ -9301,22 +9549,22 @@ proxy-addr@~2.0.5:
...
+  resolved "https://registry.npmjs.org/public-encrypt/-/public-encrypt-4.0.3.tgz"
   ...
   dependencies:
     bn.js "^4.1.0"
@@ -9328,7 +9576,7 @@ public-encrypt@^4.0.0:
...
+  resolved "https://registry.npmjs.org/query-string/-/query-string-4.3.4.tgz"
   ...
   dependencies:
     object-assign "^4.1.0"
@@ -9396,56 +9649,56 @@ query-string@^4.1.0:
...
+  resolved "https://registry.npmjs.org/randomfill/-/randomfill-1.0.4.tgz"
   ...
   dependencies:
     randombytes "^2.0.5"
@@ -9453,12 +9706,12 @@ randomfill@^1.0.3:
...
+  resolved "https://registry.npmjs.org/raw-loader/-/raw-loader-1.0.0.tgz"
   ...
   dependencies:
     loader-utils "^1.1.0"
@@ -9476,7 +9729,7 @@ raw-loader@^1.0.0:
...
+  resolved "https://registry.npmjs.org/rc/-/rc-1.2.8.tgz"
   ...
   dependencies:
     deep-extend "^0.6.0"
@@ -9486,7 +9739,7 @@ rc@^1.2.7:
 
 react-activestorage-provider@*:
   version "0.8.0"
-  resolved "https://registry.yarnpkg.com/react-activestorage-provider/-/react-activestorage-provider-0.8.0.tgz#4e64365dc089eb1149e975f90b223ed132b23012"
+  resolved "https://registry.npmjs.org/react-activestorage-provider/-/react-activestorage-provider-0.8.0.tgz"
   integrity sha512-7I0ujhezFBV6oYaP+U4WUOwn3/3l6NAv1eWOMghp0JUgPwrj0H23MkvaDAhkycVCCAQdKmJdOcraOl+bzD/Q8w==
   dependencies:
     "@babel/runtime" "^7.1.2"
@@ -9494,7 +9747,7 @@ react-activestorage-provider@*:
 
 react-animate@^3.0.0:
   version "3.0.0"
-  resolved "https://registry.yarnpkg.com/react-animate/-/react-animate-3.0.0.tgz#6a7a0d72477ebb1bde24d42154c7b3eb4ab7f42d"
+  resolved "https://registry.npmjs.org/react-animate/-/react-animate-3.0.0.tgz"
   integrity sha1-anoNckd+uxveJNQhVMez60q39C0=
   dependencies:
     bluebird "^2.9.30"
@@ -9505,7 +9758,7 @@ react-animate@^3.0.0:
...
+  resolved "https://registry.npmjs.org/react-beautiful-dnd/-/react-beautiful-dnd-10.1.1.tgz"
   ...
   dependencies:
     "@babel/runtime-corejs2" "^7.4.2"
@@ -9517,31 +9770,38 @@ react-beautiful-dnd@^10.0.3:
     redux "^4.0.1"
...
+
 react-device-detect@^1.5.9:
   version "1.9.10"
-  resolved "https://registry.yarnpkg.com/react-device-detect/-/react-device-detect-1.9.10.tgz#3ea17e1383fef87886fb85cc850747dad306d050"
+  resolved "https://registry.npmjs.org/react-device-detect/-/react-device-detect-1.9.10.tgz"
   integrity sha512-Rb618NuNwJojJeIgI5UoONwztbd4wMT4TBofzvYOHnPNu61LYYz8b78cXQb7kXRrPn6iI5T1gFtNQh0GxHEPxw==
   dependencies:
     ua-parser-js "^0.7.20"
...
-react-dom@^16.8.6:
+react-dom@*, "react-dom@^0.14.0 || ^15.0.0 || ^16.0.0", "react-dom@^0.14.0 || ^15.0.0-rc || ^16.0.0-rc || ^16.0.0", "react-dom@^15.3.0 || ^16.0.0-alpha", "react-dom@^15.5.0 || ^16.0.0-rc", react-dom@^16.0.0, react-dom@^16.2.0, react-dom@^16.4.0, react-dom@^16.8.6, "react-dom@>= 16.3.0", "react-dom@>= 16.8.0", react-dom@>=15.0.0, "react-dom@0.14.x || ^15.0.0 || ^16.0.0":
   version "16.12.0"
...
+  resolved "https://registry.npmjs.org/react-dom/-/react-dom-16.12.0.tgz"
   ...
   dependencies:
     loose-envify "^1.1.0"
@@ -9551,17 +9811,17 @@ react-dom@^16.8.6:
 
 react-google-analytics@^0.2.0:
   version "0.2.0"
-  resolved "https://registry.yarnpkg.com/react-google-analytics/-/react-google-analytics-0.2.0.tgz#1a0b090a633051c9ad3ae05ccb4768174c2a0380"
+  resolved "https://registry.npmjs.org/react-google-analytics/-/react-google-analytics-0.2.0.tgz"
   integrity sha1-GgsJCmMwUcmtOuBcy0doF0wqA4A=
 
 react-hook-mathjax@^2.0.5:
   version "2.0.5"
-  resolved "https://registry.yarnpkg.com/react-hook-mathjax/-/react-hook-mathjax-2.0.5.tgz#bd15e5cca38cd6c3a2564b68a380beaf9f6814cd"
+  resolved "https://registry.npmjs.org/react-hook-mathjax/-/react-hook-mathjax-2.0.5.tgz"
   integrity sha512-AtOWIhrVGpO5dVplEZWf8G8Z6kGyxpNH5Xc7MPIdOm/d/o+s13ZMta+YClsZpFGbhAccpoHyeZ59YJE1IelMLQ==
...
+  resolved "https://registry.npmjs.org/react-intl/-/react-intl-2.9.0.tgz"
   ...
   dependencies:
     hoist-non-react-statics "^3.3.0"
@@ -9572,17 +9832,17 @@ react-intl@^2.2.3:
...
+  resolved "https://registry.npmjs.org/react-map-gl/-/react-map-gl-4.1.16.tgz"
   ...
   dependencies:
     "@babel/runtime" "^7.0.0"
@@ -9594,7 +9854,7 @@ react-map-gl@^4.1.1:
...
+  resolved "https://registry.npmjs.org/react-markdown/-/react-markdown-4.2.2.tgz"
   ...
   dependencies:
     html-to-react "^1.3.4"
@@ -9608,19 +9868,19 @@ react-markdown@^4.0.4:
...
+  resolved "https://registry.npmjs.org/react-medium-image-zoom/-/react-medium-image-zoom-3.1.2.tgz"
...
+  resolved "https://registry.npmjs.org/react-oembed-container/-/react-oembed-container-0.3.0.tgz"
...
+  resolved "https://registry.npmjs.org/react-popper/-/react-popper-0.8.3.tgz"
   ...
   dependencies:
     popper.js "^1.12.9"
@@ -9628,7 +9888,7 @@ react-popper@^0.8.2:
...
+  resolved "https://registry.npmjs.org/react-popper/-/react-popper-1.3.6.tgz"
   ...
   dependencies:
     "@babel/runtime" "^7.1.2"
@@ -9640,7 +9900,7 @@ react-popper@^1.3.3:
...
+  resolved "https://registry.npmjs.org/react-redux/-/react-redux-5.1.2.tgz"
   ...
   dependencies:
     "@babel/runtime" "^7.1.2"
@@ -9653,7 +9913,7 @@ react-redux@^5.0.7, react-redux@^5.1.1:
...
+  resolved "https://registry.npmjs.org/react-router-dom/-/react-router-dom-4.3.1.tgz"
   ...
   dependencies:
     history "^4.7.2"
@@ -9665,7 +9925,7 @@ react-router-dom@^4.3.1:
...
+  resolved "https://registry.npmjs.org/react-router/-/react-router-4.3.1.tgz"
   ...
   dependencies:
     history "^4.7.2"
@@ -9678,14 +9938,14 @@ react-router@^4.3.1:
...
@@ -9694,7 +9954,7 @@ react-sortable-hoc@^0.8.4:
...
+  resolved "https://registry.npmjs.org/react-spring/-/react-spring-8.0.27.tgz"
   ...
   dependencies:
     "@babel/runtime" "^7.3.1"
@@ -9702,15 +9962,15 @@ react-spring@^8.0.9:
...
+react-transition-group@^2.2.1, react-transition-group@^2.5.1:
   version "2.9.0"
...
+  resolved "https://registry.npmjs.org/react-transition-group/-/react-transition-group-2.9.0.tgz"
   ...
   dependencies:
     dom-helpers "^3.4.0"
@@ -9720,24 +9980,24 @@ react-transition-group@^2.5.1:
...
+  resolved "https://registry.npmjs.org/react-virtualized-auto-sizer/-/react-virtualized-auto-sizer-1.0.2.tgz"
 ...
 react-youtube-player@^2.0.1:
   version "2.0.1"
-  resolved "https://registry.yarnpkg.com/react-youtube-player/-/react-youtube-player-2.0.1.tgz#2ebe8a95a75ba06156b9429c3055495f6598d0fa"
+  resolved "https://registry.npmjs.org/react-youtube-player/-/react-youtube-player-2.0.1.tgz"
   integrity sha1-Lr6KladboGFWuUKcMFVJX2WY0Po=
   dependencies:
     youtube-player "^5.1.0"
 
-react@^16.8.6:
+react@*, "react@^0.13.0 || ^0.14.0 || ^15.0.0 || ^16.0.0", "react@^0.14.0 || ^15.0.0 || ^16.0.0", "react@^0.14.0 || ^15.0.0-0 || ^16.0.0-0", "react@^0.14.0 || ^15.0.0-rc || ^16.0.0-rc || ^16.0.0", "react@^0.14.9 || ^15.0.0 || ^16.0.0", "react@^15.0.0 || ^16.0.0", "react@^15.3.0 || ^16.0.0-alpha", "react@^15.5.0 || ^16.0.0-rc", react@^16.0, react@^16.0.0, react@^16.2.0, react@^16.3.1, react@^16.4.0, react@^16.8.6, "react@<= 16.x.x", "react@>= 16.3.0", "react@>= 16.8.0", react@>=15, react@>=15.0.0, react@>=16.3.0, "react@~0.13.x || ~0.14.x || ^15.0.0 || ^16.0.0 || ^17.0.0", "react@0.14.x || ^15.0.0 || ^16.0.0":
   version "16.12.0"
...
+  resolved "https://registry.npmjs.org/read-pkg-up/-/read-pkg-up-1.0.1.tgz"
   ...
   dependencies:
     find-up "^1.0.0"
@@ -9761,7 +10021,7 @@ read-pkg-up@^1.0.1:
...
+  resolved "https://registry.npmjs.org/read-pkg-up/-/read-pkg-up-2.0.0.tgz"
   ...
   dependencies:
     find-up "^2.0.0"
@@ -9769,7 +10029,7 @@ read-pkg-up@^2.0.0:
...
+  resolved "https://registry.npmjs.org/read-pkg-up/-/read-pkg-up-3.0.0.tgz"
   ...
   dependencies:
     find-up "^2.0.0"
@@ -9777,7 +10037,7 @@ read-pkg-up@^3.0.0:
...
+  resolved "https://registry.npmjs.org/read-pkg-up/-/read-pkg-up-4.0.0.tgz"
   ...
   dependencies:
     find-up "^3.0.0"
@@ -9785,7 +10045,7 @@ read-pkg-up@^4.0.0:
...
+  resolved "https://registry.npmjs.org/read-pkg/-/read-pkg-2.0.0.tgz"
   ...
   dependencies:
     load-json-file "^2.0.0"
@@ -9803,16 +10063,16 @@ read-pkg@^2.0.0:
...
+  resolved "https://registry.npmjs.org/readable-stream/-/readable-stream-2.3.6.tgz"
   ...
   dependencies:
     core-util-is "~1.0.0"
@@ -9823,9 +10083,18 @@ read-pkg@^3.0.0:
...
+  resolved "https://registry.npmjs.org/readable-stream/-/readable-stream-3.4.0.tgz"
   ...
   dependencies:
     inherits "^2.0.3"
@@ -9834,7 +10103,7 @@ readable-stream@^3.0.6, readable-stream@^3.1.1:
...
+  resolved "https://registry.npmjs.org/readdirp/-/readdirp-2.2.1.tgz"
   ...
   dependencies:
     graceful-fs "^4.1.11"
@@ -9843,14 +10112,14 @@ readdirp@^2.2.1:
...
+  resolved "https://registry.npmjs.org/recompose/-/recompose-0.30.0.tgz"
   ...
   dependencies:
     "@babel/runtime" "^7.0.0"
@@ -9862,7 +10131,7 @@ recompose@^0.30.0:
...
+  resolved "https://registry.npmjs.org/reduce-css-calc/-/reduce-css-calc-1.3.0.tgz"
   ...
   dependencies:
     balanced-match "^0.4.2"
@@ -9894,7 +10163,7 @@ reduce-css-calc@^1.2.7:
...
+  resolved "https://registry.npmjs.org/reduce-css-calc/-/reduce-css-calc-2.1.7.tgz"
   ...
   dependencies:
     css-unit-converter "^1.1.1"
@@ -9902,31 +10171,31 @@ reduce-css-calc@^2.0.0:
...
+"redux@^2.0.0 || ^3.0.0 || ^4.0.0-0", redux@^4.0.0, redux@^4.0.1, redux@>=1.0.0:
   version "4.0.4"
...
+  resolved "https://registry.npmjs.org/redux/-/redux-4.0.4.tgz"
   ...
   dependencies:
     loose-envify "^1.4.0"
@@ -9934,36 +10203,36 @@ redux@^4.0.0, redux@^4.0.1:
...
+  resolved "https://registry.npmjs.org/regex-not/-/regex-not-1.0.2.tgz"
   ...
   dependencies:
     extend-shallow "^3.0.2"
@@ -9971,19 +10240,19 @@ regex-not@^1.0.0, regex-not@^1.0.2:
...
+  resolved "https://registry.npmjs.org/regexpu-core/-/regexpu-core-4.6.0.tgz"
   ...
   dependencies:
     regenerate "^1.4.0"
@@ -9995,19 +10264,19 @@ regexpu-core@^4.6.0:
...
+  resolved "https://registry.npmjs.org/remark-parse/-/remark-parse-5.0.0.tgz"
   ...
   dependencies:
     collapse-white-space "^1.0.2"
@@ -10028,7 +10297,7 @@ remark-parse@^5.0.0:
...
+  resolved "https://registry.npmjs.org/remark-parse/-/remark-parse-6.0.3.tgz"
   ...
   dependencies:
     collapse-white-space "^1.0.2"
@@ -10049,7 +10318,7 @@ remark-parse@^6.0.0:
...
+  resolved "https://registry.npmjs.org/remark-stringify/-/remark-stringify-6.0.4.tgz"
   ...
   dependencies:
     ccount "^1.0.0"
@@ -10069,7 +10338,7 @@ remark-stringify@^6.0.0:
...
+  resolved "https://registry.npmjs.org/remarkable/-/remarkable-1.7.1.tgz"
   ...
   dependencies:
     argparse "~0.1.15"
@@ -10086,50 +10355,50 @@ remarkable@1.7.1:
...
+  resolved "https://registry.npmjs.org/request/-/request-2.88.0.tgz"
   ...
   dependencies:
     aws-sign2 "~0.7.0"
@@ -10155,39 +10424,39 @@ request@^2.87.0, request@^2.88.0:
...
+  resolved "https://registry.npmjs.org/resolve-dir/-/resolve-dir-1.0.1.tgz"
   ...
   dependencies:
     expand-tilde "^2.0.0"
@@ -10195,46 +10464,46 @@ resolve-dir@^1.0.0, resolve-dir@^1.0.1:
...
+  resolved "https://registry.npmjs.org/restore-cursor/-/restore-cursor-2.0.0.tgz"
   ...
   dependencies:
     onetime "^2.0.0"
@@ -10242,51 +10511,56 @@ restore-cursor@^2.0.0:
...
+rimraf@^2.5.4, rimraf@^2.6.3, rimraf@^2.7.1, rimraf@2:
   version "2.7.1"
...
+  resolved "https://registry.npmjs.org/ripemd160/-/ripemd160-2.0.2.tgz"
   ...
   dependencies:
     hash-base "^3.0.0"
@@ -10294,60 +10568,65 @@ ripemd160@^2.0.0, ripemd160@^2.0.1:
...
+  resolved "https://registry.npmjs.org/sass-graph/-/sass-graph-2.2.4.tgz"
   ...
   dependencies:
     glob "^7.0.0"
@@ -10372,7 +10651,7 @@ sass-graph@^2.2.4:
...
+  resolved "https://registry.npmjs.org/sass-loader/-/sass-loader-7.3.1.tgz"
   ...
   dependencies:
     clone-deep "^4.0.1"
@@ -10383,7 +10662,7 @@ sass-loader@7.3.1:
...
+  resolved "https://registry.npmjs.org/sass-loader/-/sass-loader-8.0.0.tgz"
   ...
   dependencies:
     clone-deep "^4.0.1"
@@ -10394,12 +10673,12 @@ sass-loader@8.0:
...
+  resolved "https://registry.npmjs.org/scheduler/-/scheduler-0.18.0.tgz"
   ...
   dependencies:
     loose-envify "^1.1.0"
@@ -10407,16 +10686,32 @@ scheduler@^0.18.0:
...
+  resolved "https://registry.npmjs.org/schema-utils/-/schema-utils-2.5.0.tgz"
   ...
   dependencies:
     ajv "^6.10.2"
@@ -10424,7 +10719,7 @@ schema-utils@^2.0.0, schema-utils@^2.0.1, schema-utils@^2.1.0, schema-utils@^2.5
...
+  resolved "https://registry.npmjs.org/scss-tokenizer/-/scss-tokenizer-0.2.3.tgz"
   ...
   dependencies:
     js-base64 "^2.1.8"
@@ -10432,34 +10727,42 @@ scss-tokenizer@^0.2.3:
...
+  resolved "https://registry.npmjs.org/sentence-case/-/sentence-case-2.1.1.tgz"
   ...
   dependencies:
     no-case "^2.2.0"
@@ -10486,17 +10789,17 @@ sentence-case@^2.1.0:
...
+  resolved "https://registry.npmjs.org/serve-index/-/serve-index-1.9.1.tgz"
   ...
   dependencies:
     accepts "~1.3.4"
@@ -10509,7 +10812,7 @@ serve-index@^1.9.1:
...
+  resolved "https://registry.npmjs.org/serve-static/-/serve-static-1.14.1.tgz"
   ...
   dependencies:
     encodeurl "~1.0.2"
@@ -10519,12 +10822,12 @@ serve-static@1.14.1:
...
+  resolved "https://registry.npmjs.org/set-value/-/set-value-2.0.1.tgz"
   ...
   dependencies:
     extend-shallow "^2.0.1"
@@ -10534,22 +10837,22 @@ set-value@^2.0.0, set-value@^2.0.1:
...
+  resolved "https://registry.npmjs.org/sha.js/-/sha.js-2.4.11.tgz"
   ...
   dependencies:
     inherits "^2.0.1"
@@ -10557,19 +10860,19 @@ sha.js@^2.4.0, sha.js@^2.4.8:
...
+  resolved "https://registry.npmjs.org/sharkdown/-/sharkdown-0.1.1.tgz"
   ...
   dependencies:
     cardinal "~0.4.2"
@@ -10578,43 +10881,43 @@ sharkdown@^0.1.0:
...
+  resolved "https://registry.npmjs.org/should/-/should-6.0.3.tgz"
   ...
   dependencies:
     should-equal "0.3.1"
@@ -10623,34 +10926,34 @@ should@^6.0.3:
...
+  resolved "https://registry.npmjs.org/slice-ansi/-/slice-ansi-2.1.0.tgz"
   ...
   dependencies:
     ansi-styles "^3.2.0"
@@ -10659,14 +10962,14 @@ slice-ansi@^2.1.0:
...
+  resolved "https://registry.npmjs.org/snapdragon-node/-/snapdragon-node-2.1.1.tgz"
   ...
   dependencies:
     define-property "^1.0.0"
@@ -10675,14 +10978,14 @@ snapdragon-node@^2.0.1:
...
+  resolved "https://registry.npmjs.org/snapdragon/-/snapdragon-0.8.2.tgz"
   ...
   dependencies:
     base "^0.11.1"
@@ -10696,7 +10999,7 @@ snapdragon@^0.8.1:
...
+  resolved "https://registry.npmjs.org/sockjs-client/-/sockjs-client-1.4.0.tgz"
   ...
   dependencies:
     debug "^3.2.5"
@@ -10708,7 +11011,7 @@ sockjs-client@1.4.0:
...
+  resolved "https://registry.npmjs.org/sockjs/-/sockjs-0.3.19.tgz"
   ...
   dependencies:
     faye-websocket "^0.10.0"
@@ -10716,19 +11019,19 @@ sockjs@0.3.19:
...
+  resolved "https://registry.npmjs.org/source-map-resolve/-/source-map-resolve-0.5.2.tgz"
   ...
   dependencies:
     atob "^2.1.1"
@@ -10739,7 +11042,7 @@ source-map-resolve@^0.5.0, source-map-resolve@^0.5.2:
...
+  resolved "https://registry.npmjs.org/source-map-support/-/source-map-support-0.5.16.tgz"
   ...
   dependencies:
     buffer-from "^1.0.0"
@@ -10747,34 +11050,39 @@ source-map-support@^0.5.6, source-map-support@~0.5.12:
...
@@ -10782,12 +11090,12 @@ spdx-correct@^3.0.0:
...
+  resolved "https://registry.npmjs.org/spdx-expression-parse/-/spdx-expression-parse-3.0.0.tgz"
   ...
   dependencies:
     spdx-exceptions "^2.1.0"
@@ -10795,12 +11103,12 @@ spdx-expression-parse@^3.0.0:
...
+  resolved "https://registry.npmjs.org/spdy-transport/-/spdy-transport-3.0.0.tgz"
   ...
   dependencies:
     debug "^4.1.0"
@@ -10812,7 +11120,7 @@ spdy-transport@^3.0.0:
...
+  resolved "https://registry.npmjs.org/static-extend/-/static-extend-0.1.2.tgz"
   ...
   dependencies:
     define-property "^0.2.5"
@@ -10900,24 +11208,24 @@ static-extend@^0.1.1:
...
+  resolved "https://registry.npmjs.org/stimulus/-/stimulus-1.1.1.tgz"
   ...
   dependencies:
     "@stimulus/core" "^1.1.1"
@@ -10925,7 +11233,7 @@ stimulus@^1.1.0:
...
+  resolved "https://registry.npmjs.org/stream-browserify/-/stream-browserify-2.0.2.tgz"
   ...
   dependencies:
     inherits "~2.0.1"
@@ -10933,7 +11241,7 @@ stream-browserify@^2.0.1:
...
@@ -10941,7 +11249,7 @@ stream-each@^1.1.0:
...
+  resolved "https://registry.npmjs.org/stream-http/-/stream-http-2.8.3.tgz"
   ...
   dependencies:
     builtin-status-codes "^3.0.0"
@@ -10952,17 +11260,31 @@ stream-http@^2.7.2:
...
+  resolved "https://registry.npmjs.org/string-length/-/string-length-2.0.0.tgz"
   ...
   dependencies:
     astral-regex "^1.0.0"
@@ -10970,16 +11292,32 @@ string-length@^2.0.0:
...
+  resolved "https://registry.npmjs.org/string-width/-/string-width-2.1.1.tgz"
   ...
   dependencies:
     is-fullwidth-code-point "^2.0.0"
@@ -10987,7 +11325,7 @@ string-width@^1.0.1, string-width@^1.0.2:
...
+  resolved "https://registry.npmjs.org/string-width/-/string-width-3.1.0.tgz"
   ...
   dependencies:
     emoji-regex "^7.0.1"
@@ -10996,7 +11334,7 @@ string-width@^3.0.0, string-width@^3.1.0:
...
+  resolved "https://registry.npmjs.org/string.prototype.trimleft/-/string.prototype.trimleft-2.1.0.tgz"
   ...
   dependencies:
     define-properties "^1.1.3"
@@ -11004,29 +11342,15 @@ string.prototype.trimleft@^2.1.0:
...
+  resolved "https://registry.npmjs.org/stringify-entities/-/stringify-entities-1.3.2.tgz"
   ...
   dependencies:
     character-entities-html4 "^1.0.0"
@@ -11036,62 +11360,74 @@ stringify-entities@^1.0.1:
...
+  resolved "https://registry.npmjs.org/style-loader/-/style-loader-1.0.0.tgz"
   ...
   dependencies:
     loader-utils "^1.2.3"
@@ -11099,12 +11435,12 @@ style-loader@^1.0.0:
...
+styled-components@^4.1.2, "styled-components@>= 2":
   version "4.4.1"
...
+  resolved "https://registry.npmjs.org/styled-components/-/styled-components-4.4.1.tgz"
   ...
   dependencies:
     "@babel/helper-module-imports" "^7.0.0"
@@ -11123,7 +11459,7 @@ styled-components@^4.1.2:
...
+  resolved "https://registry.npmjs.org/stylehacks/-/stylehacks-4.0.3.tgz"
   ...
   dependencies:
     browserslist "^4.0.0"
@@ -11132,14 +11468,14 @@ stylehacks@^4.0.0:
 
 stylelint-color-format@^0.2.0:
   version "0.2.0"
-  resolved "https://registry.yarnpkg.com/stylelint-color-format/-/stylelint-color-format-0.2.0.tgz#b0bb2ce504d66da98564bead3f6c170d8d7b1284"
+  resolved "https://registry.npmjs.org/stylelint-color-format/-/stylelint-color-format-0.2.0.tgz"
   integrity sha512-DXe7TZUahf2MENrRc3mT3qEGGYlQtuns/u8d+EC7oFnzIghEKvY/9Y6u7epkTGmpxgBRm534mi5LIgj9pWDPCQ==
   dependencies:
     color "^3.0.0"
 
-stylelint@^9.10.1:
+"stylelint@^8.0.0 || ^9.0.0", stylelint@^9.10.1:
   version "9.10.1"
...
+  resolved "https://registry.npmjs.org/stylelint/-/stylelint-9.10.1.tgz"
   ...
   dependencies:
     autoprefixer "^9.0.0"
@@ -11192,62 +11528,76 @@ stylelint@^9.10.1:
...
+  resolved "https://registry.npmjs.org/swap-case/-/swap-case-1.1.2.tgz"
   ...
   dependencies:
     lower-case "^1.1.1"
@@ -11274,17 +11624,17 @@ swap-case@^1.1.0:
...
+  resolved "https://registry.npmjs.org/table/-/table-5.4.6.tgz"
   ...
   dependencies:
     ajv "^6.10.2"
@@ -11294,12 +11644,12 @@ table@^5.0.0, table@^5.2.3:
...
+  resolved "https://registry.npmjs.org/terser-webpack-plugin/-/terser-webpack-plugin-1.4.1.tgz"
   ...
   dependencies:
     cacache "^12.0.2"
@@ -11336,7 +11684,7 @@ terser-webpack-plugin@^1.4.1:
...
+  resolved "https://registry.npmjs.org/terser-webpack-plugin/-/terser-webpack-plugin-2.2.1.tgz"
   ...
   dependencies:
     cacache "^13.0.1"
@@ -11350,7 +11698,7 @@ terser-webpack-plugin@^2.2.1:
...
+  resolved "https://registry.npmjs.org/terser/-/terser-4.4.0.tgz"
   ...
   dependencies:
     commander "^2.20.0"
@@ -11359,7 +11707,7 @@ terser@^4.1.2, terser@^4.3.9:
...
+  resolved "https://registry.npmjs.org/test-exclude/-/test-exclude-5.2.3.tgz"
   ...
   dependencies:
     glob "^7.1.3"
@@ -11369,62 +11717,62 @@ test-exclude@^5.2.3:
...
+  resolved "https://registry.npmjs.org/title-case/-/title-case-2.1.1.tgz"
   ...
   dependencies:
     no-case "^2.2.0"
@@ -11432,41 +11780,41 @@ title-case@^2.1.0:
...
+  resolved "https://registry.npmjs.org/to-regex-range/-/to-regex-range-2.1.1.tgz"
   ...
   dependencies:
     is-number "^3.0.0"
@@ -11474,7 +11822,7 @@ to-regex-range@^2.1.0:
...
+  resolved "https://registry.npmjs.org/to-regex/-/to-regex-3.0.2.tgz"
   ...
   dependencies:
     define-property "^2.0.2"
@@ -11484,12 +11832,12 @@ to-regex@^3.0.1, to-regex@^3.0.2:
...
+  resolved "https://registry.npmjs.org/tough-cookie/-/tough-cookie-2.5.0.tgz"
   ...
   dependencies:
     psl "^1.1.28"
@@ -11497,7 +11845,7 @@ tough-cookie@^2.3.3, tough-cookie@^2.3.4:
...
+  resolved "https://registry.npmjs.org/tough-cookie/-/tough-cookie-2.4.3.tgz"
   ...
   dependencies:
     psl "^1.1.24"
@@ -11505,90 +11853,90 @@ tough-cookie@~2.4.3:
...
+  resolved "https://registry.npmjs.org/tunnel-agent/-/tunnel-agent-0.6.0.tgz"
 ...
 tween-interpolate@^1.0.1:
   version "1.0.2"
-  resolved "https://registry.yarnpkg.com/tween-interpolate/-/tween-interpolate-1.0.2.tgz#829df1529daf0962b8a036efac76c57f36eb83cf"
+  resolved "https://registry.npmjs.org/tween-interpolate/-/tween-interpolate-1.0.2.tgz"
   integrity sha1-gp3xUp2vCWK4oDbvrHbFfzbrg88=
...
+  resolved "https://registry.npmjs.org/type-is/-/type-is-1.6.18.tgz"
   ...
   dependencies:
     media-typer "0.3.0"
@@ -11596,27 +11944,27 @@ type-is@~1.6.17, type-is@~1.6.18:
...
+  resolved "https://registry.npmjs.org/uglify-js/-/uglify-js-3.6.9.tgz"
   ...
   dependencies:
     commander "~2.20.3"
@@ -11624,17 +11972,17 @@ uglify-js@^3.1.4:
...
+  resolved "https://registry.npmjs.org/unherit/-/unherit-1.1.2.tgz"
   ...
   dependencies:
     inherits "^2.0.1"
@@ -11642,12 +11990,12 @@ unherit@^1.0.4:
...
+  resolved "https://registry.npmjs.org/unicode-match-property-ecmascript/-/unicode-match-property-ecmascript-1.0.4.tgz"
   ...
   dependencies:
     unicode-canonical-property-names-ecmascript "^1.0.4"
@@ -11655,17 +12003,17 @@ unicode-match-property-ecmascript@^1.0.4:
...
+  resolved "https://registry.npmjs.org/unified/-/unified-7.1.0.tgz"
   ...
   dependencies:
     "@types/unist" "^2.0.0"
@@ -11691,12 +12039,12 @@ unified@^7.0.0:
...
+  resolved "https://registry.npmjs.org/union-value/-/union-value-1.0.1.tgz"
   ...
   dependencies:
     arr-union "^3.1.0"
@@ -11706,99 +12054,99 @@ union-value@^1.0.0:
...
+  resolved "https://registry.npmjs.org/unset-value/-/unset-value-1.0.0.tgz"
   ...
   dependencies:
     has-value "^0.3.1"
@@ -11806,36 +12154,36 @@ unset-value@^1.0.0:
...
+  resolved "https://registry.npmjs.org/url-loader/-/url-loader-1.1.2.tgz"
   ...
   dependencies:
     loader-utils "^1.1.0"
@@ -11844,7 +12192,7 @@ url-loader@^1.1.1:
...
+  resolved "https://registry.npmjs.org/url-parse/-/url-parse-1.4.7.tgz"
   ...
   dependencies:
     querystringify "^2.1.1"
@@ -11852,7 +12200,7 @@ url-parse@^1.4.3:
...
+  resolved "https://registry.npmjs.org/url/-/url-0.11.0.tgz"
   ...
   dependencies:
     punycode "1.3.2"
@@ -11860,61 +12208,61 @@ url@^0.11.0:
 
 use-immer@^0.1.0:
   version "0.1.0"
-  resolved "https://registry.yarnpkg.com/use-immer/-/use-immer-0.1.0.tgz#58b2f861f539faa9fdc8e695cda9202e0c8f1997"
...
+  resolved "https://registry.npmjs.org/validate-npm-package-license/-/validate-npm-package-license-3.0.4.tgz"
   ...
   dependencies:
     spdx-correct "^3.0.0"
@@ -11922,22 +12270,22 @@ validate-npm-package-license@^3.0.1:
...
+  resolved "https://registry.npmjs.org/verror/-/verror-1.10.0.tgz"
   ...
   dependencies:
     assert-plus "^1.0.0"
@@ -11946,12 +12294,12 @@ verror@1.10.0:
...
+  resolved "https://registry.npmjs.org/vfile-message/-/vfile-message-2.0.2.tgz"
   ...
   dependencies:
     "@types/unist" "^2.0.0"
@@ -11959,14 +12307,14 @@ vfile-message@*:
...
+  resolved "https://registry.npmjs.org/viewport-mercator-project/-/viewport-mercator-project-6.2.2.tgz"
   integrity sha512-vs9pjDZxDwoiEqr+GgdpRAUogEQRoZ9D9COVLSr4+h/WdcUWhYN36SQr3n5jaTKrl/wlfdRj+TYU8wTk05grbg==
   dependencies:
     "@babel/runtime" "^7.0.0"
@@ -11999,12 +12347,12 @@ viewport-mercator-project@^6.2.1:
...
+  resolved "https://registry.npmjs.org/vt-pbf/-/vt-pbf-3.1.1.tgz"
   ...
   dependencies:
     "@mapbox/point-geometry" "0.1.0"
@@ -12013,33 +12361,33 @@ vt-pbf@^3.1.1:
...
+  resolved "https://registry.npmjs.org/watchpack/-/watchpack-1.6.0.tgz"
   ...
   dependencies:
     chokidar "^2.0.2"
@@ -12048,19 +12396,19 @@ watchpack@^1.6.0:
...
+  resolved "https://registry.npmjs.org/webpack-assets-manifest/-/webpack-assets-manifest-3.1.1.tgz"
   ...
   dependencies:
     chalk "^2.0"
@@ -12073,7 +12421,7 @@ webpack-assets-manifest@^3.1.1:
...
+  resolved "https://registry.npmjs.org/webpack-bundle-analyzer/-/webpack-bundle-analyzer-3.6.0.tgz"
   ...
   dependencies:
     acorn "^6.0.7"
@@ -12092,7 +12440,7 @@ webpack-bundle-analyzer@^3.0.3:
...
+  resolved "https://registry.npmjs.org/webpack-cli/-/webpack-cli-3.3.10.tgz"
   ...
   dependencies:
     chalk "2.4.2"
@@ -12109,7 +12457,7 @@ webpack-cli@^3.3.0, webpack-cli@^3.3.10:
...
+  resolved "https://registry.npmjs.org/webpack-dev-middleware/-/webpack-dev-middleware-3.7.2.tgz"
   ...
   dependencies:
     memory-fs "^0.4.1"
@@ -12120,7 +12468,7 @@ webpack-dev-middleware@^3.7.2:
...
+  resolved "https://registry.npmjs.org/webpack-dev-server/-/webpack-dev-server-3.9.0.tgz"
   ...
   dependencies:
     ansi-html "0.0.7"
@@ -12159,7 +12507,7 @@ webpack-dev-server@^3.1.10:
...
+  resolved "https://registry.npmjs.org/webpack-log/-/webpack-log-2.0.0.tgz"
   ...
   dependencies:
     ansi-colors "^3.0.0"
@@ -12167,22 +12515,22 @@ webpack-log@^2.0.0:
...
-webpack@^4.27.1, webpack@^4.41.2:
+"webpack@^2.0.0 || ^3.0.0 || ^4.0.0", "webpack@^3.0.0 || ^4.0.0", webpack@^4.0.0, webpack@^4.27.1, webpack@^4.3.0, webpack@^4.36.0, webpack@^4.4.0, webpack@^4.41.2, webpack@>=2, webpack@>=4.4.0, webpack@4.x.x:
   version "4.41.2"
...
+  resolved "https://registry.npmjs.org/webpack/-/webpack-4.41.2.tgz"
   ...
   dependencies:
     "@webassemblyjs/ast" "1.8.5"
@@ -12211,7 +12559,7 @@ webpack@^4.27.1, webpack@^4.41.2:
...
+  resolved "https://registry.npmjs.org/websocket-driver/-/websocket-driver-0.7.3.tgz"
   ...
   dependencies:
     http-parser-js ">=0.4.0 <0.4.11"
@@ -12220,39 +12568,39 @@ websocket-driver@>=0.5.1:
...
+  resolved "https://registry.npmjs.org/whatwg-url/-/whatwg-url-6.5.0.tgz"
   ...
   dependencies:
     lodash.sortby "^4.7.0"
@@ -12261,7 +12609,7 @@ whatwg-url@^6.4.1:
...
+  resolved "https://registry.npmjs.org/whatwg-url/-/whatwg-url-7.1.0.tgz"
   ...
   dependencies:
     lodash.sortby "^4.7.0"
@@ -12270,48 +12618,48 @@ whatwg-url@^7.0.0:
...
+which@^1.2.14, which@^1.2.9, which@^1.3.0, which@^1.3.1, which@1:
   version "1.3.1"
...
+  resolved "https://registry.npmjs.org/wrap-ansi/-/wrap-ansi-2.1.0.tgz"
   ...
   dependencies:
     string-width "^1.0.1"
@@ -12319,7 +12667,7 @@ wrap-ansi@^2.0.0:
...
+  resolved "https://registry.npmjs.org/wrap-ansi/-/wrap-ansi-5.1.0.tgz"
   ...
   dependencies:
     ansi-styles "^3.2.0"
@@ -12328,12 +12676,12 @@ wrap-ansi@^5.1.0:
...
+  resolved "https://registry.npmjs.org/write-file-atomic/-/write-file-atomic-2.4.1.tgz"
   ...
   dependencies:
     graceful-fs "^4.1.11"
@@ -12342,96 +12690,99 @@ write-file-atomic@2.4.1:
...
+  resolved "https://registry.npmjs.org/yaml-jest/-/yaml-jest-1.0.5.tgz"
...
+  resolved "https://registry.npmjs.org/yargs-parser/-/yargs-parser-11.1.1.tgz"
   ...
   dependencies:
     camelcase "^5.0.0"
@@ -12439,7 +12790,7 @@ yargs-parser@^11.1.1:
...
+  resolved "https://registry.npmjs.org/yargs-parser/-/yargs-parser-13.1.1.tgz"
   ...
   dependencies:
     camelcase "^5.0.0"
@@ -12447,49 +12798,14 @@ yargs-parser@^13.1.0, yargs-parser@^13.1.1:
